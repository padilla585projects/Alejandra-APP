...
| **F-1.3-NUCLEO-PAQUETES-NPM** | Reestructurar `nucleo-cognitivo/` en monorepo con `@alejandra/cognitive-core` y `@alejandra/cognitive-core-policy` publicables en npm | **Estructura lista; publicación pendiente** (2026-08-07) | Commit `a9b7db1`. Workspaces en `package.json` raíz. Cada paquete con `lib.js`, `verify_nucleo.sh`, `package.json`, `README.md`, `src/index.js`. `ci.yml` actualizado (sintaxis + verify + tests ambos paquetes). Workflow `publish-nucleo.yml` listo (manual, requiere `NPM_TOKEN`). Shim transitorio en `nucleo-cognitivo/src/motor-decision.js` para no romper `alejandra-agente/worker.js`. Tests: cognitive-core 35/35, cognitive-core-policy 4/4, agente 161/161. **Pendiente: ejecutar publicación en npm** (secreto `NPM_TOKEN` + cuenta `@alejandra`). |
