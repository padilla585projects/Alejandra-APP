...
- **Lote 7 completado (2026-08-02)** — notificaciones y generación de contenido: `enviar_email` y `enviar_telegram_informe` (**N2** — salen literalmente de la organización, el ejemplo textual de ADR-0006); `enviar_push`, `iniciar_conversacion`, `controlar_app` (**N1** — se quedan dentro del ecosistema propio de la app vía FCM/comandos, acotados por `puedeNotificarUsuario`, distinto de "salir de la organización"); `generar_informe`, `subir_archivo`, `ram_save`, `ram_clear` (**N1** — escritura de un solo archivo/fila, scratch propio o informe sin enviar). 120/120 pruebas en verde. **54/103 tools migradas**; quedan 15 en el agente y 34 en `worker.js` raíz.
- Siguiente acción exacta: revisar las 15 tools restantes del agente una por una (candidatas: `analizar_foto`, `generar_esquema`, `listar_esquemas`, `borrar_esquema`, `generar_grafico`, `preguntar_usuario`, `generar_plano`, `generar_documento`, `historico_materiales`, `exportar_datos`) — no asumir N1 por defecto. Después: el catálogo de `worker.js` raíz (34 tools, gating independiente).
