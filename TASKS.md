...
| **F-2.2-NEXO-V1** | Implementar Nexo v1 (ADR-0021): registro de fuentes, metadata en tools, trazas, telemetría, fallback coordinado | **Implementado; migración D1 pendiente de aplicar** (2026-08-07) | `nexo-fuentes.js` (3 fuentes, helpers), metadata `nexo` en `buscar_normativa`/`buscar_precios`, `registrarNexoConsulta()` con validación de fuente registrada, fallback `sugerencia:'buscar_web'`, migración `migrate_013` añadida a workflow + manifiesto, 7 tests (168/168). Requiere aplicar migración D1 vía `migrate-d1-agent.yml` (autorización humana). |
