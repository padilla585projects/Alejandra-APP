...
Ninguna tarea de migración de catálogo de tools sigue activa: **F-1.3-MIGRAR-RESTO-TOOLS se completó el 2026-08-02** (ver tabla de completadas). **Ocho verticales de ARC-011 fase 3 completos** (`checklists`, `rfis`, `calidad`, `tareas_obra`, `actas_reunion`, `ordenes_cambio`, `ordenes_compra`, `proveedores_gestion`), ciclo de 5 pasos cerrado en todos. **Seis verticales más del tercer lote agrupado** (ver `ARC-011-FASE3-LOTE3` abajo) tienen pasos 1-3 completos (declarar, aplicar contra D1 real el 2026-08-03, y retirar el DDL en runtime, fusionado a `main` en PR #75 el mismo día); solo falta el paso 4 (desplegar y verificar), a la espera de que el Director abra la ventana de despliegue. **Nota operativa (2026-08-03):** el Director señaló que se estaban acumulando demasiados despliegues seguidos en poco tiempo (5 en <14h); los siguientes lotes deben espaciarse más y agrupar más verticales por despliegue — este lote de 6 sigue ese criterio (un único despliegue para verificar los 6 verticales de una vez, cuando se abra la ventana).
...
- Estado: **paso 1 (declarar) y paso 2 (aplicar) completos en los 6 (2026-08-03), verificados columna por columna contra D1 real; paso 3 (retirar DDL en runtime) completo en código (2026-08-03, rama `feat/arc011-lote3-retirar-ddl-runtime`, pendiente de PR/merge); paso 4 (verificar en producción, desplegando `worker.js` una sola vez para los 6) pendiente de que el Director abra la ventana de despliegue**
- Prioridad: Media
...
- Criterios de aceptación:
  ...
  6. ⬜ Paso 4 (verificar en producción tras desplegar, agrupando los 6): pendiente de que el Director abra la ventana de despliegue. `main` ya tiene el código sin el DDL en runtime, pero **no está desplegado todavía** — hasta el despliegue, producción sigue sirviendo la versión anterior (con el DDL activo, sin riesgo).
...
- Bloqueos: paso 4 pendiente de que el Director abra una ventana separada de despliegue (decisión operativa de espaciar, no autorización de dato).
