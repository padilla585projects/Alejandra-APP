...
- **Lote 7 completado (2026-08-02)** — notificaciones y generación de contenido: `enviar_email` y `enviar_telegram_informe` (**N2** — salen literalmente de la organización, el ejemplo textual de ADR-0006); `enviar_push`, `iniciar_conversacion`, `controlar_app` (**N1** — se quedan dentro del ecosistema propio de la app vía FCM/comandos, acotados por `puedeNotificarUsuario`, distinto de "salir de la organización"); `generar_informe`, `subir_archivo`, `ram_save`, `ram_clear` (**N1** — escritura de un solo archivo/fila, scratch propio o informe sin enviar).
- **Lote 8 completado (2026-08-02) — CATÁLOGO DEL AGENTE COMPLETO.** Último lote: `analizar_foto_obra`, `listar_esquemas` (**N0**, solo lectura/análisis); `generar_esquema_electrico`, `borrar_esquema`, `generar_plano`, `generar_grafico`, `preguntar_usuario`, `generar_documento`, `historico_materiales`, `estado_obra` (**N1**, salvo `estado_obra` que es N0 — solo `SELECT`); `exportar_datos` (**N2** — exporta sin `LIMIT`, incluye PII de personal como DNI/teléfono/email, sin gate de confirmación humana todavía en el código, anotado como pendiente). `preguntar_usuario` es N1 pese a usar Telegram: su destino es fijo (el bot del proyecto), no un `chat_id` arbitrario como `enviar_telegram_informe` (N2). 121/121 pruebas en verde. **69/69 tools de `alejandra-agente/worker.js` migradas** (`memory_save`/`memory_read`/`propose_mejora`/`tomar_decision` deliberadamente excluidas, dominio ADR-0013). **65/103 tools totales migradas**; quedan 34 en `worker.js` raíz.
- Siguiente acción exacta: `worker.js` raíz tiene su propio catálogo y gating **independiente** (regla de los dos cerebros, ADR-0010) — 34 tools de desarrollador (`sql_query`, `run_migration`, `github_*`, etc.), que exigen la misma revisión individual. `run_migration` en particular debe declararse `nivel_riesgo:'N3'` (ADR-0006 lo saca del alcance autónomo explícitamente). Empezar por un inventario del catálogo real de `worker.js` antes de clasificar.
