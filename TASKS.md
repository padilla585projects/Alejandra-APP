...
- Progreso: **lote 2 completado (2026-08-02)** — 8 tools de solo lectura migradas (`buscar_documentos`, `buscar_tareas`, `consultar_inventario`, `buscar_precios`, `buscar_procedimientos`, `consultar_punch_list`, `buscar_proveedores`, `consultar_precios`), todas `acceso:'sesion'`, `cron:'permitido'`, `nivel_riesgo:'N0'`. **Lote 3 completado (2026-08-02)** — 7 tools públicas migradas (`buscar_web`, `calcular_cable`, `calcular_bandeja`, `calcular_proteccion`, `pensar`, `planificar`, `buscar_normativa`), todas `acceso:'publico'`, `cron:'permitido'`, `nivel_riesgo:'N0'`. 16 tools migradas de 103 (piloto + lote 2 + lote 3); quedan 53 en el agente y 34 en `worker.js` raíz.
- Siguiente acción exacta: elegir el siguiente lote. Ya no quedan tools N0 obvias sin tocar; el siguiente lote entra en escritura reversible (N1, ADR-0006) — candidatas a revisar una por una: `gestionar_tarea`, `gestionar_rfi`, `gestionar_oc`, `gestionar_acta`, `gestionar_calidad`, `marcar_plano`, `editar_plano` (todas en `TOOLS_REQUIEREN_SESION`, ninguna en `TOOLS_SOLO_DEV_VERIFICADO`/`TOOLS_PROHIBIDAS_CRON`) — verificar cada una individualmente que es escritura acotada al propio usuario/obra antes de asignarle N1, no asumir por el nombre.
