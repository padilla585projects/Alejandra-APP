...
- Última actualización: 2026-08-03
- Siguiente acción exacta: **pospuesta por decisión del Director.** Se retoma cuando el proyecto entre en fase estable de preproducción/producción; mientras tanto se mantiene la configuración a nivel de repositorio y ningún agente maneja los valores reales. Checklist de referencia lista en `docs/runbooks/CHECKLIST-F02-CFG-SECRETOS-ENTORNO.md` para cuando el Director decida ejecutarla personalmente.
...
| fix/panel-alejandra-chat-sync-drag-resize | Fix del salto del chat de Alejandra en `panel.html` + mover/redimensionar | Completada (2026-08-03) | PR #76. Bug real reportado por Adrián: el sondeo cada 5s repintaba toda la ventana del chat aunque solo hubiera un mensaje nuevo (incluido de otra plataforma, vía `alejandra_historial` compartida). `cargarAlejandraChat()` ahora compara firmas y solo añade mensajes nuevos al final. Único archivo tocado: `panel.html`. **Pendiente de comprobar paridad en `index.html`/`alejandra-panel.html`** (no forma parte de este entregable). |
