...
- Siguiente acción exacta: revisar el resto de tools de escritura del agente una por una (candidatas: `escribir_bd`, `enviar_push`, `enviar_email`, `subir_archivo`, `generar_plano`, `generar_esquema_electrico`, `generar_documento`, `iniciar_conversacion`, etc.) — algunas de estas son N2 por definición de ADR-0006 (envío fuera de la organización, escritura amplia) o exigen revisión de a qué Set pertenecen (`TOOLS_SOLO_DEV_VERIFICADO` → `acceso:'dev_verificado'`); no asumir N1 por defecto. Después: el catálogo de `worker.js` raíz (34 tools, gating independiente).
