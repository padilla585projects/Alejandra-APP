...
Estado (actualizado 2026-08-07): **No hay ninguna tarea activa, en curso ni bloqueada.** Todas las entradas de este documento están `completada`/`cerrado`/`desplegada y verificada`, incluidas las 14 verticales de ARC-011 fase 3, F-0.2-CFG (secretos por entorno + ensayo + política de rama, 2026-08-04), F-1.1/F-1.2/F-1.3 (Época 1 completa) y F-2.1-MEMORIA-ESCRITURA (Época 2). ARC-014 sigue como riesgo aceptado sin acción de ingeniería (revisada 2026-08-03, sin cambios); **ARC-021** (bypass de despliegue de `alejandra-agente` vía `wrangler deploy` directo) se acepta como práctica habitual, mismo criterio (2026-08-07, ver arriba). Trabajo más reciente: SEC-CHAT-CONTEXTO-LEGACY + ADR-0020 rebanada 1 (2026-08-06), F-1.3 núcleo cognitivo v2 en subcarpetas locales, F-2.2 Nexo v1 (ADR-0021) y ADR-0020 rebanada 2 — piloto N0 ampliado a las 36 tools (2026-08-07). **Único pendiente real, sin tarea abierta todavía:** rebanada 3 de ADR-0020 (verificadores de lectura N1 en `verifier.js`), que exige ADR/enmienda + aprobación del Director antes de empezar — ver `ARCHITECT_BACKLOG.md` (ARC-020) y `OPEN_TASK_SUMMARY.md`. Dos fixes fuera del roadmap ya cerrados (panel.html PR #76, 2026-08-03; 4 bugs de `index.html` vía sugerencias con foto, 2026-08-04) — detalle en `PROJECT_STATE.md`/`CHANGELOG.md`. No contiene tareas ficticias; `MASTER_ROADMAP.md` mantiene el plan global y `ARCHITECT_BACKLOG.md` mantiene deuda/propuestas.
