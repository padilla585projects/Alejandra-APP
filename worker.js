  ...
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      if (path.startsWith('/actas-reunion/')) {
        const _aid = parseInt(path.split('/actas-reunion/')[1]);
        if (method === 'GET')    return await getActaReunionById(_aid, request, env);
  ...
  try {
    ...
    try {
      ...
      if (subcVencen && subcVencen.length) {
        const porEmpSub = {};
        for (const s of subcVencen) {
          if (!porEmpSub[s.empresa_id]) porEmpSub[s.empresa_id] = [];
...
async function ensureFotosObraExtended(env) {
  ...
  for (const col of [
    'ALTER TABLE fotos_obra ADD COLUMN titulo TEXT',
...
async function listarFotosObra(request, env) {
  ...
  const fecha  = url.searchParams.get('fecha') || null;
...
async function subirFotoObra(request, env) {
  ...
  const titulo     = (form.get('titulo') || '').trim() || null;
  const comentario = (form.get('comentario') || form.get('descripcion') || '').trim() || null;
  const tags       = (form.get('tags') || '').trim() || null;
  const ubicacion  = (form.get('ubicacion') || '').trim() || null;
  const fecha_foto = form.get('fecha') || new Date().toISOString().slice(0,10);
  const ts         = Date.now();
  const safeName   = file.name.replace(/[^a-zA-Z0-9._-]/g, '_');
  const r2Key      = `e${empresa_id}/galeria/${obra_id || 0}/${ts}_${safeName}`;
  await env.FILES.put(r2Key, await file.arrayBuffer(), { httpMetadata: { contentType: mime } });
  const r = await env.DB.prepare(`
    INSERT INTO fotos_obra (empresa_id, obra_id, departamento, r2_key, nombre, mime_type, tamano, comentario, subido_por, titulo, tags, ubicacion, fecha_foto)
    VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)
  `).bind(empresa_id, obra_id, dept, r2Key, file.name, mime, file.size || null,
          comentario, userNombre || rol, titulo, tags, ubicacion, fecha_foto).run();
  return json({ ok: true, id: r.meta.last_row_id }, 201);
}
...
async function actualizarActaReunion(id, request, env) {
  ...
  await ensureActasTable(env);
  const b = await request.json().catch(()=>({}));
  // Normalize asistentes/pendientes
  if (b.asistentes !== undefined && typeof b.asistentes === 'object') b.asistentes = JSON.stringify(b.asistentes);
  if (b.pendientes  !== undefined && typeof b.pendientes  === 'object') b.pendientes  = JSON.stringify(b.pendientes);
  if (b.puntos_tratados !== undefined && !b.resumen) b.resumen = b.puntos_tratados;
  const campos = ['titulo','tipo','fecha','hora','lugar','convocante','asistentes','resumen','puntos_tratados',
                  'acuerdos','proxima_reunion','estado','orden_dia','pendientes','redactor','updated_at'];
  const sets=[]; const params=[];
  for (const c of campos) {
    if (c === 'updated_at') { sets.push(`${c}=datetime('now')`); continue; }
    if (b[c] !== undefined) { sets.push(`${c}=?`); params.push(b[c]); }
  }
