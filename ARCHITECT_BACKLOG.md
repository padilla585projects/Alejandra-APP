...
| ARC-012 | Pendiente | Tres columnas ausentes en producción por DDL silenciado | Alto | Derivado de ARC-011 fase 2. `planos.circuitos_json` rompe 4 operaciones de planos; `inventario_seg.ubicacion` mantiene abierto SEG-01 pese a darse por cerrado el 25/07/2026; `empresas.retencion_config` impide configurar y aplicar la retención de datos. Las tres se arreglan con `ADD COLUMN`, riesgo bajo, pero requieren autorización de migración. La causa raíz —18 `catch` vacíos que ocultan fallos de DDL— exige despliegue y debe resolverse para que no se repita. |
