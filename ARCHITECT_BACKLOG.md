...
| ARC-017 | Pendiente | El cron del agente ejecuta el modelo con privilegios de desarrollador | **Crítico** | `alejandra-agente` corre un cron 6 veces al día (`0 5,8,11,14,17,19`) que llama a `procesarConNEXUS(..., authOk=true, esDevVerificado=true)` (`worker.js:3987`), de modo que `filtrarToolsPorAuth` **no filtra nada**: el modelo alcanza `ejecutar_deploy`, `rollback`, `patch_codigo`, `github_escribir` y `escribir_bd` con el aislamiento por empresa desactivado, sin ningún humano delante. El motivo por defecto de un despliegue es «Deploy autónomo por Alejandra». Es una decisión deliberada («identidad ya confiada por diseño»), no un descuido, por eso NO se ha tocado. Responde a la pregunta 3 de ADR-0006: hoy el cron **no** está limitado a N0–N1, alcanza N3. **No concluyente:** si el clasificador enruta en la práctica los prompts del cron a expertos con esas tools — depende del runtime y solo los logs de producción lo aclaran. |
