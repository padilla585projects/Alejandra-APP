...
| ARC-008 | Cerrado (diseño) | Observabilidad y métricas cognitivas | Medio | **ADR-0014 aceptado con modificaciones el 2026-08-02**: tabla D1 `alejandra_trazas` compartida por los dos Workers, discriminada por `tipo`; retención 30 días para trazas de decisión y 90 días para errores de DDL/eventos de seguridad, con minimización/redacción obligatoria (nunca conversaciones completas); un único endpoint `GET /admin/trazas` en `alejandra-app-api`; `/health` con tres estados (`healthy`/`degraded`/`unhealthy`) comprobando D1 y un objeto centinela en R2, versión derivada del SHA de despliegue. Migración autorizada **solo en el entorno actual de desarrollo/pruebas**, con copia previa y validación posterior — no se extiende a producción real. Desbloquea que `nucleo-cognitivo/motor-decision.js` reciba el helper `registrarTraza()` como interfaz; la persistencia real exige además aplicar esa migración. |
