...
| ARC-011 | Inventariado (fase 1) | Esquema D1 definido por DDL en tiempo de ejecución | Crítico | **105 de 150 tablas existen solo porque el código las crea**: el esquema de producción no se puede reconstruir desde el repositorio. 173 sentencias DDL ejecutables (147 en `worker.js`, 26 en el agente), 18 con el error silenciado. `schema_completo.sql` cubre menos de un tercio del esquema real. Inventario completo en `docs/architecture/07-INVENTARIO-DDL-RUNTIME.md`. Fase 2: contraste con D1 real (requiere autorización de solo lectura). Fase 3: ADR, migrador único y retirada del DDL en runtime por verticales. |
