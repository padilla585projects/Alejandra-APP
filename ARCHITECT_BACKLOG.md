...
| ARC-013 | Pendiente | 18 `catch` vacíos ocultan fallos de DDL | Alto | Causa raíz de ARC-012: cada `ALTER TABLE` en runtime lleva `.catch(() => {})` o `try/catch` vacío, de modo que un fallo no deja rastro y produce un bug silencioso. Tres de dieciocho ya habían fallado sin que nadie lo supiera. Sustituir la supresión por registro de error. Exige despliegue de `worker.js`, por lo que no entró en el arreglo de datos. |
