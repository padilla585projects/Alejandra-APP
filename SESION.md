...
**Sesion:** LIBRE
**Fecha:** 25/07/2026 -- Sesión de pruebas en vivo en producción con Alberto (Bobinas + chat interno). Varios bugs reales y críticos encontrados y arreglados en caliente: devolución de bobinas/PEMP/carretillas rota de raíz (SCAN-01), longitud nunca se guardaba (SCAN-02), fechas "Invalid Date" en todo Office (SCAN-03), botones flotantes solapados (SCAN-04) y movibles (SCAN-04b), chat interno mostrando mensajes al revés (SCAN-05) y — el más grande — el chat nunca fue un chat de verdad: cada usuario solo veía sus propios mensajes (CHAT-01, reescrito con canal "Todos" + privado 1 a 1). v8.27.
**Versión actual:** v8.27 (version.json/sw.js/index.html sincronizados)
**Resumen:** Sesión anterior sobre roles y departamentos (Parts 50-56, hasta v8.23) — ver más abajo. Después, Adrián pidió probar producción con Alberto metiendo bobinas en vivo; salieron muchos bugs reales seguidos, arreglados uno tras otro sin parar el hilo de pruebas (ver Part 57 para el detalle completo). Ver Parts 50-57.

**PENDIENTE (checklist para retomar):**
- **Chat interno**: la app móvil (index.html) cae correctamente al canal "Todos" con el nuevo backend, pero todavía no tiene selector de contacto para chat privado 1 a 1 — solo se construyó en Office. Pendiente dar paridad a la app.
- **Notificaciones del chat**: confirmado con Adrián que se quiere notificación push cuando llega un mensaje nuevo, pero se priorizó "que funcione primero" — no construido todavía.
- **`#remoteScanFab` no es arrastrable** (a propósito, por ser un contenedor con sub-botones — ver Part 57) — si Adrián también lo quiere movible, hace falta más cuidado para no romper el click de sus hijos.
- **`_parseFechaBackend()` (SCAN-03) es un parche en el formateador de Office**, no en el origen (`fechaEspana()` en worker.js sigue guardando fechas en formato español no-ISO) — funciona, pero cualquier fecha nueva que se muestre sin pasar por `fmtFecha()`/`fmtHora()` puede seguir dando problemas. Revisar si merece la pena migrar `fechaEspana()` a ISO con una migración de datos.
- **Departamentos sin menú propio en Office todavía:** Obra Civil, Albañilería, Pintura, Carpintería, Telecom, Almacén — sin confirmación de que tengan cuentas reales usándolos; preguntar a Adrián antes de construir a ciegas.
- **Ingeniería** sigue con menú por departamento (`organizarSidebarIngenieria`), no por el rol "Ingeniero" nuevo — decidir si se alinea al mismo patrón rol+departamento del resto o se deja como está.
- **SEG-02 (Registro de Seguridad) sin probar con login real todavía** — todo lo verificado fue simulando sesión en el navegador; falta que Katherine (o cualquier persona de Seguridad) lo use de verdad desde su cuenta.
- **Asunciones sin confirmar explícitamente por Adrián, a revisar si hace falta**: Recursos Preventivos oculta Carnets/Reconocimientos (Part 51); Operario en Eléctrico/Mecánicas oculta también Trabajadores (Part 56, ORG-05) — ambas por privacidad, sin pedirlo campo a campo.
- **Resend sigue en dominio sandbox** (`noreply@resend.dev`) — probable que no entregue a destinatarios externos reales; afecta al email de Pedidos (Part 38) y al nuevo de Registro de Seguridad (Part 53). Pendiente verificar un dominio propio en Resend.
- **Sugerencias de la app**: no se ha vuelto a revisar el log desde la #208 (Partes de trabajo, cerrada en Part 41) — puede haber entradas nuevas de Adrián sin mirar.
