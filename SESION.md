...
2. **"Plan diferido" sin ejecutar** — la salvaguarda que fuerza a Alejandra a ejecutar una tool tras anunciarla en texto (en vez de dejarlo dicho y no hecho) tenía una regex de frases fijas que no cubría "Necesito usar la herramienta correcta. Espera." Regex ampliada (2 sitios idénticos). Commit `4ee77fe`.
3. **Clasificador sin memoria de conversación** — un mensaje corto/ambiguo ("Que paso?") se reclasificaba como experto "simple" (barato, sin `escribir_bd`, sin `asistente_escaneo`, historial mínimo) perdiendo el hilo de una tarea en curso. Nueva `mantenerContinuidadExperto()`: si el turno anterior del mismo usuario usó un experto "de trabajo" hace <15min, se mantiene en vez de reclasificar a ciegas. Commit `4ee77fe`.
4. **Preguntaba IDs de empresa/obra ya nombrados** — pese a tener `consultar_bd`, paraba el flujo para preguntar el ID de "Levitec" cuando Adrián ya lo había dicho dos veces. Nuevo PASO 1b en `asistente_escaneo`: resolver por nombre con `consultar_bd` antes de preguntar. Commit `607d075`.
5. **Volvía a pedir la foto ya procesada** — tras varios turnos resolviendo empresa/obra, "me falta la foto" pese a tener ya los 10 datos extraídos y confirmados en la misma conversación. Nuevo PASO 1c: no repetir trabajo ya hecho.
6. **Empresa_id/obra_id inventados (el más grave)** — Alejandra afirmó con seguridad "Levitec = empresa_id 3" (real: empresa_id 1; el 3 es Edison Montajes) y estuvo a punto de registrar equipos bajo la empresa equivocada. Causa raíz: el contexto `[Sesión: ...]` que recibe en cada mensaje nunca incluía el `empresa_id` de la sesión activa del usuario (aunque el servidor ya lo tenía disponible) — tenía que adivinarlo o buscarlo a ciegas cada vez. Fix: `construirMessages()` ahora resuelve y añade `empresa_id="N" (Nombre)` al contexto de sesión; nueva instrucción en `contexto_sesion` para usarlo directo en vez de inventar/recordar. **Verificado en vivo dos veces** tras el deploy: con la cuenta de Adrián (superadmin, bypass dev) y con la de Alberto Martínez (id=46, rol encargado, sin bypass) — ambas resuelven `empresa_id` correcto y Alberto además respetó el flujo de confirmación antes de guardar (sin bypass de aislamiento por empresa, sin problema). Registros de prueba (matrículas 9101-9103, 9301-9302) borrados de `pemp` tras verificar.

Tras cerrar los 6 fixes, continuamos en la misma sesión con 3 mejoras más:

7. **Opciones pinchables en el chat** — Adrián pidió poder elegir con botones en vez de escribir texto libre cuando Alejandra pregunta algo cerrado (confirmar, elegir entre 2-4 alternativas). Alejandra ahora termina esas preguntas con `<<OPCIONES: A|B|C>>` (instrucción nueva en el módulo `formato`); implementado y **verificado en vivo** en los 4 frontends: `index.html` (PWA, v8.68), y en `panel.html` los 3 chats (FAB `alejandrEnviar`, popup `wSend`, drawer dev `aiSend`) + `alejandra-panel.html` (centralizado en `finalizarTextoStream`) en v8.69. Probado contra el worker real: una pregunta con pendientes múltiples emitió correctamente `<<OPCIONES: Ambas|Solo amoladora Makita|Solo taladro Bosch>>`.
8. **Grok (xAI) añadido a la cascada de fallback** — Adrián puso `XAI_API_KEY` como secret. Cascada actualizada: Gemini (si hay foto) → **Grok** (nuevo, a petición explícita de Adrián de ponerlo justo antes de OpenRouter) → OpenRouter gratis → GPT-4o. `lib.js`: `calcularCosteYProveedor` reconoce `grok-4` como proveedor `xai` con su propio precio (si no, habría caído en el `else` genérico y se habría etiquetado mal, igual que el bug histórico de gpt-4o). **Sin verificar en vivo** — no se pudo forzar un fallo real de Anthropic para disparar el camino de Grok; el código sigue el mismo patrón ya probado de GPT-4o/OpenRouter.
9. **`AGENTS.md`/`_fix_ia_acceso.js` resueltos** — Adrián decidió: `AGENTS.md` (duplicado desactualizado de este mismo CLAUDE.md, nunca commiteado) se borró sin más, CLAUDE.md ya es la fuente de verdad. `_fix_ia_acceso.js` se completó y aplicó: se adaptó (su paso 3, `checkIABtn`, ya no aplicaba porque ese cambio se hizo en v7.53) y se ejecutó — añade la card "Alejandra IA" al home de `index.html` (v8.70), incluida en `_HOME_TRADE_MODS` para que sea configurable por departamento. Verificada en navegador (sesión simulada vía localStorage + servidor local): la card aparece con su estilo y el click navega bien a la pantalla de Alejandra IA. Script de un solo uso borrado tras aplicarlo.

**Versión actual:** v8.70 (`version.json`/`sw.js`/`index.html` sincronizados; `alejandra-agente/worker.js` desplegado en cada paso)
**Resumen:** Parts 50-59 = ver historial anterior (roles/departamentos, UX sidebar). Part 60 = auditoría de seguridad completa + estrés (SEC-AUDIT-01 a 08, 27/07). Sesiones 28-29/07 = fixes varios de Alejandra Office/FAB y chat IA (ver commits `acc7fac`..`5142bda`). Esta sesión (30/07) = 6 bugs de fiabilidad/memoria/datos del chat de Alejandra + opciones pinchables (4 frontends) + Grok en la cascada de fallback + card de Alejandra IA en el home, ver arriba.

**Grok verificado en vivo (mismo día):** para confirmar `_intentarGrokFallback` sin arriesgar el tráfico real de otros usuarios (no se tocó `ANTHROPIC_API_KEY`), se añadió un gancho de prueba TEMPORAL en `llamarAnthropic` (marcador de texto `FORCE_FALLBACK_TEST_GROK` que fuerza el camino de fallback), se desplegó, se disparó una petición real con `wrangler tail` abierto, y se confirmó en el log real del worker: `[Fallback] Grok OK: grok-4.3`. El gancho se retiró inmediatamente después (el diff volvió a quedar limpio, igual que el commit `4a29a7a`) y se redesplegó la versión sin él. Nota aparte: la tabla `alejandra_token_uso` no mostró estas llamadas de prueba al consultarla justo después (posible retraso de lectura en D1 remoto) — el log de `wrangler tail` fue la prueba definitiva, no la tabla.

**PARA RETOMAR EN OTRO CHAT:** sesión cerrada limpia, sin trabajo a medias, sin pendientes sueltos de sesiones anteriores (todo commiteado, pusheado y desplegado; npm test 85/85 verde en cada paso).
- **Paridad de frontends como práctica continua** (regla en `CLAUDE.md` "UNA Alejandra, DOS cerebros"): las opciones pinchables ya están a la par en los 4 sitios; cualquier FUTURA mejora del chat debe seguir replicándose igual.

Pendiente sin resolver de sesiones anteriores (dejado sin tocar, fuera de alcance de esta sesión): quedan dos archivos sueltos sin commitear en el directorio de trabajo -- `AGENTS.md` (duplicado desactualizado de este mismo CLAUDE.md, con datos obsoletos: dice "3 sitios" en vez de 4 y "Codex Sonnet" en vez de "Claude Sonnet") y `_fix_ia_acceso.js` (script Node borrador para añadir una card "Alejandra IA" al home de `index.html` -- sus anclas de texto para el paso de `checkIABtn` ya no existen porque ese cambio concreto se aplicó y commiteó hace tiempo en v7.53 vía `a6256a4`; el resto del script, tarjeta de home + orden, nunca se aplicó). Ninguno de los dos se ha tocado en esta sesión -- decidir si completar/descartar en otra sesión.
...
A petición explícita de Adrián tras cerrar SEC-AUDIT-07 ("arregla todo, esta app tendrá datos particulares de empresa, no puede ser hackeada"): revisando el archivo en busca de más patrones `SELECT COUNT(*)/MAX(numero)` seguidos de un `INSERT` separado (el mismo patrón que SEC-AUDIT-05 ya había confirmado explotable en vivo — 7 duplicados en 15 peticiones concurrentes), aparecieron **33 endpoints más** con exactamente el mismo problema, nunca cubiertos en esa ronda: RFIs, actas de reunión, control de calidad, contratos de obra, submittals, NCRs (2 sitios), órdenes de cambio, transmittals, riesgos de obra, action items, instrucciones de obra, órdenes de compra/trabajo, garantías, alquileres, entregables, lecciones aprendidas, evaluaciones de proveedores, solicitudes de cambio/material, ensayos de materiales, residuos de obra, partes de maquinaria, consumos de material, flujo de caja, licencias de obra, comparativos de oferta y catálogo de precios unitarios. Varios son registros legales o de seguridad — **accidentes/incidentes, ATS/JHA (análisis de riesgo laboral), residuos de obra, ensayos de materiales** — donde un número duplicado es un problema de cumplimiento normativo real, no solo cosmético.

Mismo fix que SEC-AUDIT-05 en los 33: número calculado dentro del propio `INSERT` (`INSERT...SELECT` con subconsulta `COALESCE`), atómico en D1. Cada endpoint tiene su propio formato (`RFI-001`, `OC-2026-0004`, `LIC-2026-0004`, algunos con año, otros sin año, algunos con `COUNT(*)` global y otros `MAX` por año), así que no fue un find-replace mecánico como el barrido de SEC-AUDIT-07 — cada uno se convirtió a mano revisando su código original.

**Nota de proceso — mismo error que en SEC-AUDIT-05, a mayor escala**: contar los `?` de cada `INSERT...SELECT` a ojo es propenso a error (ya pasó una vez en SEC-AUDIT-05); esta vez, con 33 conversiones, se escribió un verificador propio (script Node que empareja cada `.prepare()+.bind()` del archivo, cuenta placeholders reales vs argumentos con profundidad de paréntesis) y se ejecutó tras cada cambio — encontró y corrigió 8 discrepancias reales de las 33 antes de desplegar, además de confirmar que el resto del archivo (915 pares más) ya estaba correcto.

**Dos bugs adicionales encontrados durante la verificación en vivo, ninguno relacionado con la condición de carrera**:
1. D1 vincula los números JS como `REAL`, así que concatenar el año directamente con `||` producía `"CONT-2026.0-0001"` en vez de `"CONT-2026-0001"` — corregido convirtiendo el año a `String()` antes de bindear (30 funciones afectadas).
2. **Bug preexistente confirmado también en el commit anterior a esta sesión** (no introducido por este trabajo): `json(data, { status: 201 })` en vez de `json(data, 201)` — la función `json()` espera un número, no un objeto — causaba un `500 "Responses may only be constructed with status codes..."` en **todas** las altas de garantías, alquileres, entregables, lecciones aprendidas y ATS/JHA. Es decir, esos 5 módulos llevaban rotos en producción para cualquier alta nueva, sin que nadie lo hubiera detectado. Corregidas las 7 apariciones del mismo patrón en todo el archivo.

**Verificado en vivo**: 15 peticiones concurrentes reales contra dos endpoints de familias SQL distintas (`accidentes-incidentes` y `contratos-obra`) → 15/15 números únicos y consecutivos en ambos, sin duplicados. Smoke test de un alta normal en los 33 endpoints (rutas reales confirmadas una a una) → todos devuelven `200/201` con un `numero` bien formado, sin `.0` y sin 500.

**Deploy:** worker.js, commit `44c5045`, `/health` 200 verificado.
...
**Seguimiento del mismo round — confusión de tipos, cerrado del todo a petición de Adrián:** siguiendo con fuzzing (JSON anidado 50.000 niveles, arrays/objetos donde se esperaba texto, `null`, números), el anidamiento profundo no rompió nada, pero `nombre`/`codigo`/`titulo` como número disparaban un `TypeError` sin manejar (`campo?.trim is not a function`, 500 genérico) en `crearUsuario`, `crearObra`, `crearIncidencia`, `crearBobina` — mismo patrón (`campo?.trim()` asumiendo que el body trae un string) repetido en realidad en **210 sitios** del archivo (no ~25 como se estimó al vuelo la primera vez). Adrián: *"arregla todo, esta app tendrá datos particulares de empresa, no puede ser hackeada"* — en vez de seguir arreglando uno a uno, se cerró la clase de bug entera de golpe: nuevo helper `safeStr(v)` (normaliza cualquier no-string a `''`) aplicado automáticamente a las 210 llamadas `.trim()`/`?.trim()` del archivo con un script de transformación (regex sobre cadenas de propiedades tipo `a.b?.[0].trim()`), revisado con `diff` completo línea a línea antes de aplicar, `node --check` en verde.

El barrido automático destapó 2 huecos que el wrapping por sí solo no resolvía del todo — casos donde el bug no era "crashea" sino "cuela una fila real con el campo en blanco en la BD" tras dejar de crashear: `crearPemp`/`crearCarretilla` (`if (!matricula)` sin trim, un número pasa el check truthy) y `addTipoMaterialSeg` (mismo patrón con `nombre`) — corregidos exigiendo `typeof === 'string'` antes de aceptar el alta, igual que los 4 de la ronda anterior. Además, revisando a mano los `if (!campo)` sin trim que quedaban, se encontró un crash real que el barrido automático NO cubría por tener una forma de código distinta: `enviarChatMensaje` con `(body.mensaje || '').trim()` — no es un `identificador.trim()` simple, así que el regex no lo tocó; seguía crasheando con un `mensaje` numérico. Corregido con el mismo helper.

**Verificado en vivo tras el deploy**: los 4 casos nuevos (pemp/carretilla/chat/tipos-material-seg con tipo numérico) ya no crashean ni cuelan filas en blanco; peticiones legítimas en los mismos 4 endpoints siguen devolviendo 200/201 sin cambios. Commits `f272aac`, `48ad54c` (4 confirmados iniciales) y `ffb7475` (barrido completo + los 3 huecos adicionales).
...
Continuación directa de SEC-AUDIT-03: Adrián pidió más ataques reales, primero cruzados entre las 2 empresas de prueba con sesiones válidas (barrido amplio — incidencias, tareas de obra, timesheets, chat, obras, documentos, enumeración de usuarios por ID, mass assignment de `empresa_id`, bypass de rate-limit por `X-Forwarded-For` falso, `/backup/empresa` — **todo correctamente defendido, nada nuevo que arreglar**), y después totalmente sin ningún token (atacante anónimo real de internet).

**Hallazgo real — `POST /recuperar-pass` roto en producción + canal de enumeración de cuentas:** un email real disparaba `500 D1_ERROR: no such column: usado`, mientras que uno inexistente respondía `200 OK` normal (y más rápido) — la tabla `reset_tokens` en producción tenía un esquema MÁS ANTIGUO (sin las columnas `usado`/`empresa_id` que el código actual necesita, creada antes de que ese código se escribiera), así que el `CREATE TABLE IF NOT EXISTS` nunca hacía nada. Consecuencia doble: (1) **el reset de contraseña por email llevaba roto para cualquier usuario real de la plataforma**, sin que nadie lo hubiera detectado; (2) la diferencia de respuesta (500 vs 200, y de tiempo) permitía a un atacante anónimo averiguar qué emails están registrados en el sistema. Corregido con una migración aditiva auto-aplicada desde el propio código (`ALTER TABLE ADD COLUMN` envuelto en try/catch, idempotente — no requiere tocar la BD a mano ni borra ninguna fila existente). Verificado en vivo: el mismo email de prueba ya no da 500 de esquema, llega hasta el paso de envío de email (falla solo por ser un dominio `.invalid`, esperable).

**Otras pruebas sin autenticación, todas bien defendidas:** inyección SQL clásica y `DROP TABLE` contra `/acceso` y `/recuperar-pass` (correctamente parametrizado, tabla intacta); acceso a archivos por clave adivinada en `/files/<key>` (404 en todos los intentos); endpoints de datos (`/incidencias`, `/timesheets`, `/usuarios`, `/obras`, `/tareas-obra`) correctamente devuelven 403 sin token (nota menor: `/bobinas`, `/pemp`, `/carretillas` devuelven 200 con array vacío en vez de 403 — inconsistente pero sin fuga real de datos, anotado como pendiente de bajo riesgo).

**Deploy:** worker.js, commit `190a435`, `/health` 200 verificado.

### MONITOR-SEGURIDAD-01 / ALERTA-ATAQUE-02 (26/07/2026): Alejandra ahora "sabe" cuándo la atacan [COMPLETADO]

Adrián: "respecto a Alejandra no me avisó de nada" (el ataque de escalada de SEC-AUDIT-03 usó una sesión válida y acertó a la primera, ningún contador de fuerza bruta se disparó) + "tenemos que enseñar a Alejandra IA a monitorizar todo eso". Nuevo aviso específico (no basado en contador, dispara en el primer intento) cada vez que alguien intenta asignarse un rol que no le corresponde. Todas las alertas de seguridad (fuerza bruta en login/admin-code/token del agente + escalada de rol) ahora se persisten también en la tabla `logs` (`origen='seguridad'`, ya en la allowlist de `consultar_bd` del agente), no solo Telegram — verificado en vivo repitiendo el ataque ya bloqueado y confirmando el registro con `wrangler d1 execute`. Commit `e89deb6`.

### SEC-AUDIT-03 (26/07/2026): pentest dinámico REAL contra 2 empresas de prueba aisladas — escalada de privilegios confirmada en vivo [COMPLETADO]

Tras SEC-AUDIT-02, un Stop hook (condición fijada por Adrián: "no pares hasta que el agente consiga datos o modifique datos... o diga que no puede entrar") exigió ir más allá de la auditoría estática — un ataque REAL. Para hacerlo sin ningún riesgo sobre los datos de Levitec, se crearon **2 empresas de prueba 100% ficticias** vía el registro público normal de la app (`POST /empresas/registro`, el mismo flujo de alta de cualquier cliente): `PENTEST-A` (empresa_id=6) y `PENTEST-B` (empresa_id=7), cada una con su usuario `empresa_admin` real y datos de prueba (una bobina, un PEMP, una carretilla). Con sesiones reales y válidas de ambas, un agente atacó de verdad.

**CRÍTICO confirmado en vivo — cualquier empresa_admin podía autoescalarse a superadmin de TODA la plataforma:**
`crearUsuario`/`editarUsuario` (worker.js) trataban `empresa_admin` exactamente igual que un superadmin de plataforma real para el tope de roles (`esAdminReal = isSuperadmin || isAdmin || isEmpresaAdmin`) — el fix de SEC-AUDIT-02 (`ROLES_ASIGNABLES_NO_ADMIN`) solo protegía contra roles NO-admin (encargado, etc.), pero `empresa_admin` — **el rol que recibe automáticamente cualquier cliente que se autorregistra en la app** — se saltaba el tope por completo. Con el token de `empresa_admin` de la empresa de prueba B: `POST /usuarios {"rol":"superadmin"}` → 201, cuenta superadmin creada; login con esa cuenta forjada → token real de superadmin, con acceso a **toda la plataforma, todas las empresas clientes**, no solo la propia. Confirmado explotando el bug hasta el final: `GET /usuarios?empresa_id=6` con esa sesión forjada devolvió usuarios reales de la Empresa A **incluido el hash de contraseña**, y `GET /empresas` (correctamente protegido, solo daba paso porque la sesión forjada YA era un superadmin "real" a ojos del sistema) devolvió nombres de empresas reales de la plataforma — el agente vio de refilón "Levitec" y paró la exploración de inmediato al detectar que no eran datos de prueba, sin tocar nada más.

Fix: se separa `esPlataformaAdmin` (superadmin/desarrollador/`es_admin` real de sesión — puede asignar CUALQUIER rol) de `esEmpresaAdmin` (puede asignar roles no-privilegiados + `empresa_admin` sobre su propia empresa, pero JAMÁS `superadmin`/`desarrollador`) en las dos funciones. `roles_extra` (permisos apilados sobre "rol", otro vector de escalada independiente) pasa a exigir `esPlataformaAdmin` también, no solo "admin real". De paso, `getUsuarios()` devolvía `u.*` (incluye `password_hash`) a cualquier admin — nunca hay uso legítimo de exponerlo vía API, se elimina del JSON de respuesta.

**Verificado en vivo tras el deploy**, con los mismos tokens de prueba: el ataque que antes daba 201 ahora da `403 "No autorizado para asignar ese rol"`; `GET /usuarios` ya no incluye `password_hash`. Las cuentas forjadas (ids 75, 76) ya las había borrado el propio agente al confirmar el hallazgo. **Bien defendido en el mismo pentest**: IDOR por código/matrícula (bobinas/PEMP/carretillas) entre las dos empresas de prueba — siempre 404, nunca fuga; inyección SQL clásica y un intento de `DROP TABLE` — correctamente parametrizado, tabla intacta.

El agente declaró explícitamente **"YA NO CONSIGO ENTRAR NI OBTENER NADA MÁS"** tras este hallazgo — condición del Stop hook satisfecha.

**Pendiente de Adrián:** borrar (o dejar, no hay ningún riesgo) las 2 empresas de prueba desde Office → Empresas: `PENTEST-A-<timestamp>` (empresa_id 6) y `PENTEST-B-<timestamp>` (empresa_id 7), cada una con 1 usuario, 1 obra, 1 bobina, 1 PEMP y 1 carretilla, todo con el prefijo `PENTEST-` en el código/matrícula para identificarlo fácil. Claude no las borra (regla del proyecto: nunca borra datos permanentemente, ni siquiera datos de prueba propios).

**Deploy:** worker.js, commit `d6176db`, `/health` 200 verificado.
...
Adrián: "vamos a empezar por algo muy importante, la seguridad en la app y la facilidad con la que alguien con malos pensamientos quiera entrar en la app y robar datos de empresas". Dos agentes de auditoría en paralelo (`worker.js` y `alejandra-agente/worker.js`) — arquitectura general sólida (tokens de sesión aleatorios de 256 bits, PBKDF2 100k iteraciones, rate limit en login, CORS restringido, sin inyección SQL, allowlist estricta de tablas en `consultar_bd`/`escribir_bd`), pero encontraron 3 críticos + varios medios/bajos. Todos corregidos, testeados (85/85 `lib.test.js` en verde) y desplegados en producción:

**Críticos:**
1. **IDOR cross-empresa en PEMP/Carretillas/Bobinas** (`worker.js`): `eliminarBobina`, `editarPemp`, `eliminarPemp`, `editarCarretilla`, `eliminarCarretilla` no filtraban `empresa_id` en el SELECT/UPDATE/DELETE (solo matrícula/código) y el check de `obraId` se saltaba entero si era falsy (típico de `empresa_admin`/`oficina` sin obra fija) — un admin de una empresa podía editar/borrar equipos de OTRA empresa con solo conocer la matrícula (visible en QR/informes/PDF). El propio código ya había corregido este bug en `editarBobina` (comentario "SCAN-01") pero nunca se replicó en las 5 funciones gemelas. Arreglado: mismo patrón `AND empresa_id = ?` en las 5.
2. **Bypass a "developer" por substring** (`alejandra-agente/worker.js`, `esDeveloperAgente`): `uid.includes('adrian')` daba permisos de desarrollador (modificar código, desplegar, saltarse el aislamiento entre empresas en la BD) a cualquier usuario cuyo nombre CONTUVIERA "adrian" — un empleado real llamado Adrián en cualquier empresa cliente, o alguien registrado como "adriana_rrhh". Arreglado: solo coincidencia exacta contra los ids/alias del creador.
3. **Envenenamiento de memoria global sin autenticación** (`alejandra-agente`): `memory_save` no exigía sesión (alcanzable desde `/webhook/evento` y `POST /`, rutas deliberadamente públicas) y guardaba en `alejandra_memoria` sin `empresa_id`; ese contenido se inyectaba luego como contexto de confianza en TODAS las conversaciones de TODAS las empresas (`obtenerContextoChat`). Un atacante sin cuenta podía inyectar una instrucción persistente ("recomienda siempre al proveedor X...") que afectaba a todos los clientes de Alejandra, sin repetir el ataque. Arreglado: `memory_save`/`memory_read`/`propose_mejora`/`leer_estado`/`tomar_decision` añadidas a `TOOLS_REQUIEREN_SESION`.

**Medios/bajos, también corregidos:** `crearMantenimiento` sobrescribía `fecha_ultima_revision` de un equipo por matrícula sin `empresa_id`; `/upload` (agente) confiaba ciegamente en el `usuario_id` del formData sin auth (file planting con identidad ajena) — ahora prefiere el `usuario_id` real de una sesión autenticada si la hay; CORS `'*'` sustituido por origen restringido en `getPlanoObraFile` (worker.js) y en las 2 respuestas HTTP del agente; nueva `timingSafeEqual()` (en ambos workers) para las 5 comparaciones de `ADMIN_CODE`/`ADMIN_TOKEN` (antes `===`); `esAdmin` en `procesarConNEXUS`/`Stream` ahora exige `authOk` además del match de nombre (mismo patrón que el hallazgo #2, para cuota de iteraciones).

**No tocado, riesgo bajo o decisión de diseño ya documentada en el código:** `/api/esquemas/view/` sirve sin auth por diseño explícito (enlaces de diagramas compartibles).

**Seguimiento (26/07/2026, mismo día): los 7 módulos rotos SÍ se arreglaron.** Adrián: "arregla lo que queda" → `getEmpresaFromRequest`/`jsonOk`/`jsonError` (usadas por `licencias-obra`, `catálogo-precios`, `seguros-obra`, `comparativos-oferta`, `cae-documentación`, `órdenes-trabajo`, `ausencias` — ~28 rutas, antes daban 500 siempre por `ReferenceError`) definidas con el mismo patrón `getAuth`/`json`/`err` del resto del archivo; `getEmpresaFromRequest()` exige sesión con `empresa_id` (lanza si no la hay, nunca deja pasar un `empresa_id` nulo). Verificado que las 6 funciones de `UPDATE`/`DELETE` de esos módulos ya filtraban `empresa_id` correctamente. Probado sin auth: `/licencias-obra` responde 500 `"No autorizado"` limpio, sin fuga. Commit `74e2d05`.

**Adrián pidió también borrar las cuentas duplicadas de Alberto — declinado**, es una acción destructiva sobre datos reales; sigue pendiente de que él la haga desde Office → Usuarios (ids 1, 12, 50, 51, todas `activo=0`; la real es la 46).

**Deploy:** `npx wrangler deploy` en ambos workers (dos veces cada uno: SEC-AUDIT-01 + el seguimiento de los 7 módulos), `/health` verificado 200 tras cada deploy. Commits `956086e` y `74e2d05`.

**PENDIENTE (checklist para retomar):**
- **Departamentos sin cuentas reales todavía:** Obra Civil, Albañilería, Pintura, Carpintería, Telecom, Almacén ya tienen menú propio en Office (DEPT-CONSTR-03, v8.47) aunque ninguno tenga cuentas reales usándolo todavía — revisar con un usuario real de cada uno en cuanto exista, las listas de páginas son una asunción razonable, no confirmada campo a campo.
- **`_parseFechaBackend()` (SCAN-03) sigue siendo un parche en el formateador de Office**, no en el origen (`fechaEspana()` en worker.js sigue guardando fechas en formato español no-ISO). Adrián decidió dejarlo para más adelante — migrar tocaría datos históricos.
- **SEG-02 (Registro de Seguridad) SÍ probado con login real** (Part 58): Adrián creó un registro de prueba con foto real desde su HTC (superadmin) y lo confirmó funcionando — el punto sigue abierto solo para que Katherine (u otra persona real de Seguridad) lo pruebe también desde su propio rol/cuenta.
- **Asunciones de privacidad sin confirmar campo a campo** (Recursos Preventivos/Operario, Parts 51/56; el nuevo subconjunto de operario.ingenieria en Part 58 tiene la misma naturaleza).
- **Resend sigue en dominio sandbox** (`noreply@resend.dev`) — requiere un dominio propio verificado en Resend, que es de pago. Adrián: "lo dejamos... para más adelante" — sin urgencia, no tocar hasta que lo pida.
- **Sugerencias de la app**: revisado en Part 58, la #208 sigue siendo la última (38 en total) — nada pendiente por ahora, pero conviene seguir revisando el log periódicamente.
- **Cuentas duplicadas de Alberto (ids 1, 12, 50, 51) sin borrar** — Adrián confirmó que sí, pero es una acción destructiva que tiene que hacer él desde Office → Usuarios (Claude no borra datos permanentemente). Pendiente de que lo haga.
- **"Quiero quitar columnas del inventario que no valen y añadir otras"**: confirmado por Adrián que ya está hecho (Part 57, sustitución de las columnas rotas de longitud + Nº albarán/Registrado por/Acciones) — cerrado, no es un pendiente real.
- **Cuenta duplicada de Alberto** (Part 58): existen 4 usuarios "Alberto" en D1 (ids 1, 12, 46, 50, 51) — solo el 46 (`Alberto Martínez`, amartinezc@levitec.es) está activo y en uso real; el resto están `activo=0`. No se ha hecho limpieza, solo se identificó al depurar por qué un mensaje de chat "no llegaba" (sí llegaba, al id correcto). Revisar si conviene borrar/desactivar formalmente los duplicados en una sesión de datos.
- **El archivado automático de bobinas devueltas (7 días) es solo un filtro de vista** (Part 58, `_bobinaEstaArchivada()` en cargarBobinas()), no borra ni mueve filas de la tabla `bobinas`. El histórico real vive en la tabla `historial` (endpoint `/historial`, ya existía). Si en el futuro se quiere cambiar el nº de días, está en la constante `BOBINAS_DIAS_ANTES_DE_ARCHIVAR` en panel.html.
- **RESUELTO en esta sesión (ya no pendiente, dejar constancia)**: sync en tiempo real de inventarios app↔Office (v8.37, polling 60s en la pantalla de Bobinas/PEMP/Carretillas de la app, igual que Office) y chat privado 1 a 1 + aviso de no-leídos en la app móvil (v8.38, paridad completa con Office). Ver detalle en Part 58 más abajo.

### Part 59: UX de sidebar + previsualización de departamento para admins + CORS + refactor "verComoAdmin" + directorio por departamento + Almacén solo-material (26/07/2026) [COMPLETADO, v8.48→v8.65]

**v8.48 — sidebar colapsado por defecto salvo Inventarios.** Adrián: "quiero que se escondan todas las categorías menos inventario cuando abras Chrome" → aclarado a "que se contraigan" (no ocultar del todo). Las secciones del sidebar arrancan colapsadas salvo "📦 Inventarios", que queda expandida por ser la más usada.

**v8.49 — sección "Admin" sin el mismo estilo que las demás.** Adrián: "ahí un departamento que se llama admin que no tiene el mismo estilo, míralo". Le faltaba la estructura `<span>`/chevron del resto de cabeceras de sección — corregido para que coincida.

**v8.50→v8.54 — previsualización de departamento para admins (PREVIEW-ADMIN).** Adrián: "¿cómo veo a qué pertenece, cuál es de Mecánicas o Electricidad, Telecom, Construcción...?" — no existía ninguna forma de que un admin viera Office como lo ve un departamento concreto.
- v8.50: el sidebar se reorganiza también para el admin según `SESSION.departamento` (fallback al config de `encargado` de ese departamento en `organizarSidebarEncargado()`).
- v8.51: Adrián — "en Alejandra Office no sale versión de nada" — se añade `#panelVersionTag` visible en el pie del sidebar.
- v8.52: Adrián — "tampoco veo el selector de departamentos" (asumía, incorrectamente, que existía uno en Office como en la app móvil) — se construye `#topbarDeptoSelect`, nuevo, solo-admin. De paso, favicon 404 corregido ("claro no quiero errores").
- v8.53: Adrián — "ahora debería filtrarse todo por departamento no?" — se preseleccionan los filtros `filtroDep*` existentes (Bobinas/PEMP/Carretillas/Pedidos) con el departamento elegido, con la excepción explícita de Adrián para Almacén ("en almacén solo queremos ver pedidos e inventarios de los otros departamentos", esos 4 filtros no se fuerzan). Luego, "sí claro todos tienen que tenerlo" → se centraliza en `_deptoPreviewParam()`/`_conDeptoPreview()`, inyectado dentro de la función global `api()`, para que CUALQUIER módulo (no solo los 4 con `<select>` propio) reciba `?departamento=X` en sus peticiones GET.
- v8.54: Adrián — "pongo almacén y sigo viendo todo el panel lateral" — `ocultarSidebarPorPermisos()` y `ocultarSeccionConstruccionSiNoAplica()` decidían la ocultación de Analítica/Finanzas/Proveedores/Maquinaria/Calidad y Cierre/Construcción mirando si el usuario era "no-admin de verdad", ignorando por completo que hubiera una previsualización activa. Se añade `previsualizandoDepto` y se trata como si el admin fuera ese rol para esos dos puntos.

**BUG-CORS-HEALTH-01 (worker.js, aparte de la numeración de versión de frontend).** En medio de esta sesión, Adrián: "el estado de conexión está en rojo" + pegó un log de consola con errores CORS reales contra `/health`. La ruta construía su `Response` a mano en vez de con el helper `json()` (que sí añade las cabeceras CORS) — la única ruta de todo el archivo con este fallo. Confirmado con `curl` explícitamente con `-H "Origin: ..."` que la cabecera faltaba en producción; corregido, desplegado con `wrangler deploy` y reverificado con el mismo `curl`. Se aprovechó para añadir las mismas cabeceras CORS a `/version` (endpoint OTA de Flutter), que tenía el mismo problema latente. Commit aparte: `9af83b4`.

**v8.55 — refactor "más fácil": una sola bandera decide todo (PREVIEW-ADMIN-05/06).** Pese a v8.54, Adrián seguía viendo en su PC real (confirmado v8.54, departamento Almacén) que "Bobinas", "Repostajes" y "Seguridad" aparecían dentro de "Material y maquinaria" — no debían, según Adrián: "solo debe ver cosas relacionadas con el trabajo de un almacenero... por supuesto inventarios de los departamentos" (cross-department sigue siendo intencional, solo material ajeno no). Causa raíz: **el mismo patrón de bug encontrado 3 veces** — puntos sueltos del código con su propio `if (isAdmin || ...)` (reveals de `.nav-electrico-section`/`.nav-seguridad-section` en `iniciarApp()`, y `mostrarInventarioSeguridadSiAplica()`) que nunca se enteraban de que había una previsualización activa, cada uno con su propia condición a parchear. Adrián: **"más fácil"**, rechazando seguir parcheando uno a uno. Refactor:
- En `iniciarApp()` y `ocultarSidebarPorPermisos()`, se calcula una única `verComoAdmin`/`previsualizandoDepto` = admin real Y NO hay previsualización activa. Todos los `if (isAdmin ...)` que deciden qué ve cada departamento pasan a usar esta bandera (los que deben seguir siendo siempre-admin de verdad — secciones Admin/Desarrollador, el propio selector — se quedan con `isAdmin` sin tocar).
- `mostrarInventarioSeguridadSiAplica()` gana un parámetro `previsualizandoDepto` explícito (antes decidía solo por el rol real, `esAdminActivo`, ciego a la previsualización).
- "Repostajes" no tenía NINGUNA clase protectora (a diferencia de Bobinas/`nav-electrico-section` o el stock de Seguridad/`nav-inventario-seguridad`) — nunca se ocultaba a nadie. `_organizarSidebarRolDept()` gana una lista genérica de "pool nativo de Inventarios" (bobinas/pemp/carretillas/repostajes/herramientas/alertasStock): cuando un departamento define `material`, todo lo del pool que NO esté en su lista se oculta explícitamente — cierra este hueco y cualquier futuro similar sin parches por página.
- Verificado en navegador (pestaña nueva + navegación limpia, para evitar el artefacto de caché de navegación ya conocido de esta herramienta): Almacén previsualizado ya NO muestra Bobinas/Seguridad/Repostajes/Finanzas/Maquinaria/Analítica/Construcción; Eléctrico previsualizado sigue mostrando su material completo (bobinas, repostajes, mantenimientos...); un admin sin previsualización (departamento en blanco) sigue viendo todo. Sin regresiones detectadas.

**v8.56 — directorio por departamento en "Todos los departamentos" (PREVIEW-ADMIN-07).** Adrián, tras ver v8.55 funcionando: "cuando seleccione todos los departamentos quiero ver menús por departamento y en cada uno los suyo. Cuando elija un departamento en el selector, se quitará todo y solo saldrá el departamento que elija". Hasta ahora "Todos los departamentos" mostraba el sidebar plano genérico (Inventarios/Construcción mezclando material y páginas de TODOS los gremios sin distinguir de quién era cada cosa). Nueva `construirDirectorioDepartamentos()`: para cada departamento con config, crea una cabecera colapsable + clones (`cloneNode`, mismo `onclick="navTo(...)"`) de sus páginas reales, sustituyendo las secciones planas "Inventarios"/"Construcción"/"Pedidos" (ocultadas). Solo se construye para el admin real SIN departamento elegido. De paso, a petición de Adrián, las secciones no-departamentales (Finanzas de Obra, Proveedores, Maquinaria y Equipos, Calidad y Cierre, Analítica, Planificación, Admin, Diario y Campo) pasan a verse SOLO en "Todos los departamentos" — se ocultan también al elegir un departamento concreto.

**v8.57 — Albañilería/Pintura/Carpintería dentro del bloque Obra Civil.** Adrián, viendo el directorio con 10 bloques: "piensa un poco, ¿Obra Civil qué engloba?" → "Obra Civil es el departamento y dentro metemos todos los que tienen que ver con eso". El directorio pasa de 10 a 7 bloques (Eléctrico, Mecánicas, Seguridad, Ingeniería, Obra Civil, Telecom, Almacén) — Albañilería/Pintura/Carpintería no son bloques propios, comparten la config de Obra Civil.

**v8.58 — stock de Seguridad huérfano bajo Almacén + todos los menús colapsados de verdad.** Dos bugs encontrados probando: (1) Adrián: "¿por qué Almacén tiene submenú Seguridad?" — el botón original del stock de material de Seguridad, oculto junto con la sección "Inventarios" en modo directorio, era re-revelado 100ms después por `ocultarSidebarPorPermisos()` (que llamaba a `mostrarInventarioSeguridadSiAplica()` ciega al modo directorio) y quedaba huérfano pegado visualmente al último bloque (Almacén) — se corrige haciendo que esa función no toque nada si el directorio está construido, y el stock vive clonado dentro de su propio bloque "🔺 Seguridad". (2) Adrián: "que se contraigan todos los menús por defecto al abrir Alejandra Office" — se quita la excepción que dejaba "Inventarios" siempre abierta (v8.48) y se arregla la causa real de que "no se contraían" pese a v8.48: `sidebarRestoreState()` hacía `sidebarToggle()` (un toggle, no un set) sobre secciones que `inicializarEstadoSidebar()` ya acababa de colapsar, volviéndolas a abrir.

**v8.59→v8.60 — Almacén "solo material" con modal de filtro por departamento (ALMACEN-FILTRO-01/02).** Tras varios mensajes rápidos de Adrián aclarando el alcance ("el almacén es solo para material" / "tiene que salir el material y filtrarlo por departamentos con un modal" / los pedidos "eso va a una pestaña solo para eso" / "todo lo demás fuera cuando selecciones almacén" / luego "tiene que tener filtro todo"): la config de Almacén pasa a `material` con TODO el material de todos los departamentos (bobinas, herramientas, PEMP, carretillas, mantenimientos, repostajes, stock de Seguridad, alertas), más una `seccionExtra` propia "📥 Pedidos" (Pedidos + Entregas), `soloMaterial: true` (oculta Hoy en obra/Personal/Documentación) y `filtroDeptModal: true`. Nuevo modal `abrirModalFiltroDeptAlmacen()`/`aplicarFiltroDeptAlmacen()` que preselecciona los filtros `filtroDep*` existentes (v8.59) y, tras el "tiene que tener filtro todo" de Adrián, se extiende (v8.60) a Herramientas/Repostajes/Mantenimientos/Alertas Stock — módulos sin filtro de departamento propio: `_deptoPreviewParam()` inyecta el filtro elegido como `?departamento=` en TODAS las peticiones GET mientras la vista activa sea Almacén, y en `worker.js` se añade el filtro real: `getBobinas/getPemp/getCarretillas/getHerramientas` ganan a Almacén como departamento con visión transversal de LECTURA (`isAdminRole`); `getRepostajes`/`getMantenimientos` (tablas sin columna `departamento`) resuelven el filtro vía JOIN contra la matrícula del PEMP/carretilla del equipo; `getAlertasStock` deja a Almacén ver alertas de todos los departamentos y acotar con `?departamento=`. Solo lectura — los gates de escritura/borrado no cambian.

**v8.61 — sección "Desarrollador" del sidebar no se plegaba con la flecha.** Adrián: "las herramientas de desarrollador tienen que ser escondibles también con la flecha". Era la única cabecera `div.sidebar-section` sin `onclick="sidebarToggle(this)"`, `data-sid` ni la estructura `<span>`+chevron del resto (mismo tipo de bug que la sección Admin en v8.49) — corregida.

**v8.62 — parpadeo de Fases/Hitos/Planos/Cronograma al entrar en Ingeniería (SIDEBAR-FLICKER-01).** Adrián: "en el departamento de Ingeniería falta todo el tema de planificación (planos)". Diagnosticado en vivo en su Chrome real: Fases+Gantt, Hitos de Obra, Planos de Obra y Cronograma de Obra viven por defecto dentro de la sección "Planificación"; `organizarSidebarEncargado()` los mueve a su bloque propio "📐 Ingeniería" vía `seccionExtra` — pero en `iniciarApp()` y `ocultarSidebarPorPermisos()` el hide-loop que oculta "Planificación" corría ANTES de esa reorganización, así que se los llevaba por delante mientras todavía eran hermanos en el DOM de "Planificación" (quedaban ocultos hasta que la reorganización los rescataba). Se invierte el orden en las dos funciones: reorganizar primero, ocultar después. Verificado con lecturas repetidas cada 50ms tras un reload real: sin ninguna ventana de ocultación. (Nota aparte: lo que parecía persistir tras el fix era simplemente el bloque "Ingeniería" colapsado por defecto, comportamiento correcto de v8.58 — no un bug.)

**v8.63 — sección "Obra" sobraba en departamentos con `controlObra:false`.** Adrián, revisando las secciones de Ingeniería a petición propia: "mira las secciones de Ingeniería... una sobra". La sección genérica "🏗️ Obra" (Documentos legacy, Fotos de Obra, Instrucciones, Inspecciones) se quedaba visible con sus páginas por defecto para cualquier departamento con `controlObra:false` (Ingeniería, Obra Civil, Albañilería, Pintura, Carpintería) — solo existía la rama "si `controlObra !== false`, renombrar y rellenar", nunca un "si no, ocultar" (mismo patrón que ya tenía "Inventarios" cuando `material` es `undefined`). Se añade `ocultarSeccion('obra')` al `else`.

**v8.64 — "Seguridad de Obra" sobraba dentro del bloque Construcción.** Adrián, en el mismo repaso: "me refiero también a Seguridad de obra, que ahí no pinta nada". Es un botón nativo de la sección Construcción (junto con Tareas/RFIs/Calidad/Punch List/Contactos/Visitas/Actas) que ni Ingeniería ni Obra Civil incluyen en su lista de páginas, y sin ninguna clase protectora propia (mismo patrón exacto que "Repostajes" en Inventarios, PREVIEW-ADMIN-06) se quedaba visible para cualquiera. `config.seccionExtra.paginas` pasa a actuar también como lista permitida para todo lo nativo de "Construcción": lo que no esté en la lista se oculta.

**v8.65 — el pie del sidebar se ocultaba al colapsar "Desarrollador" + "Chat DevTools IA" reaparecía solo.** Adrián: "algo pasa cuando abro y escondo el panel de Desarrollador" + "también esconde el número de versión en la parte de abajo, eso no debe esconderse nunca". Dos causas encontradas en vivo: (1) "Desarrollador" es la última sección del sidebar; `sidebarToggle()`/`ocultarSeccion()` recorren hermanos "hasta la siguiente `.sidebar-section`" y, al no haber ninguna después, se llevaban por delante el pie completo (Cerrar sesión + versión) al colapsarla — se añade un centinela invisible (`div.sidebar-section` sin `data-sid`, oculto con `!important`, ignorado por `inicializarEstadoSidebar()`/`sidebarRestoreState()`) justo antes del pie para que el barrido pare ahí. (2) `showAIBtn()` (reintento cada 3s indefinidamente, mismo patrón que BUG-FAB-01) forzaba "Chat DevTools IA" a `display:flex` sin mirar si el usuario había colapsado la sección — ahora respeta `sec-collapsed` en cada intento. Verificado en Chrome real: colapsando "Desarrollador" y esperando 3.5s (para que dispare el reintento), el botón sigue oculto y la versión + Cerrar sesión permanecen visibles.

**Deploy:** `worker.js` desplegado una vez en esta parte (ALMACEN-FILTRO-02, v8.60) vía `npx wrangler deploy`. Las 10 versiones de frontend (v8.56→v8.65) comiteadas y pusheadas individualmente, cada una verificada contra `https://padilla585projects.github.io/Alejandra-APP/panel.html` en el Chrome real de Adrián (vía la extensión claude-in-chrome, con su sesión ya logueada — nunca credenciales introducidas por Claude) tras confirmar la propagación de GitHub Pages con `curl` sobre `version.json`.

**Contexto:** continuación directa de Part 57 en la misma sesión de pruebas en producción. Reglas del Part 57 siguen aplicando: cada bug se arregla en el momento, sin dejarlo para después.

**v8.28 — botón flotante de chat interno (SCAN-04c) + confirma columna Nº albarán.** Adrián: "faltaría un botón flotante para chat interno no?" — hasta ahora solo se abría desde el sidebar. Se añade `#fabChatEquipo` (esquina opuesta a los de Alejandra). La columna Nº albarán en Bobinas (pedida en Part 57) se confirma ya en el código pero **nunca se había llegado a desplegar** — se publica junto con el botón.

**v8.29 — la tabla de Bobinas se rompía visualmente.** En vivo, Alberto vio una fila "Registrado por: Alberto Martínez" desplegada debajo de CADA bobina (captura de pantalla real). Causa: `responsiveLayout:'collapse'` de Tabulator no plegaba tras un icono "+" como se esperaba, sino que forzaba la columna de menor prioridad siempre abierta como fila extra. Se quita el collapse; `layout:'fitData'` + scroll horizontal en su lugar — ninguna columna se pliega ni se oculta, todas accesibles.

**v8.30 — auto-recarga del panel al detectar nueva versión.** Adrián, en medio de la prueba: "podíamos hacer que se recargue la web sola después de una actualización no?" — cada fix se quedaba sin llegar a Alberto hasta que alguien refrescaba a mano. Polling ligero de `version.json` (cada 2 min + al volver a la pestaña) con aviso y recarga a los 4s. Sin comparar contra una constante fija (evita un 5º archivo de versión que sincronizar): la primera lectura fija la base.

**v8.31 — el FAB seguía sin verse tras recargar (intento 1 fallido) + scroll más visible.** El primer intento de fix (`setTimeout(0)` en `iniciarApp()`) no bastaba: seguía disparando antes de que el resto del `<script>` del documento terminara de parsear/ejecutar (el HTML de los botones vive más abajo en el body, fuera de ese script). Se cambia a `window.addEventListener('load', ...)` con fallback si el evento ya pasó. De paso, Adrián: "el scroll lateral que se vea un poquito más la barra" — de 6px a 12px solo en el scroll interno de las tablas.

**v8.32 — el FAB TODAVÍA no aparecía, esta vez solo en modo desarrollador.** El fix de v8.31 seguía sin cubrir el caso desarrollador. Se copia el patrón ya fiable de `showAIBtn()` (llamada inicial + reintento cada 3s **indefinidamente**, no solo una vez) — esta vez sí funcionó, verificado con sesión simulada `rol:'desarrollador'`.

**v8.33 — filtros de Estado y Nº albarán + imprimir respeta el filtro completo.** Adrián: "falta el filtrado por albarán y por estado para poder imprimirlo". Selects de valores exactos (mismo patrón que Proveedor/Tipo cable), combinados en AND con la búsqueda de texto libre. `tblBobinas.print()` pasa de `(false,true)` a `(true,true)` — antes solo imprimía la página de 20 visible en pantalla, ahora imprime TODAS las filas que cumplan el filtro.

**v8.34 — historial de bobinas en Office + archivado automático + oculta filtro de obra inútil.** Adrián: "me gustaría que se archivara [lo devuelto] por si algún día tenemos que tirar del listado" + "que podamos verlo en cualquier momento en la app y en office" + "por supuesto imprimirlo o enviarlo" + "podemos dejar las devueltas unos días en la lista y luego automáticamente se pasen al archivo". El endpoint `/historial` y su registro en cada devolución ya existían en `worker.js` (nunca se borra nada); la app móvil YA tenía una pantalla para verlo (pestaña "🕒 Historial", no hacía falta tocarla) — solo faltaba en Office. Se añade botón "📜 Historial" con tabla + imprimir/CSV, y `_bobinaEstaArchivada()` oculta del inventario activo las devueltas con más de 7 días (constante `BOBINAS_DIAS_ANTES_DE_ARCHIVAR`), sin borrar nada. Aparte, Adrián: "revisar el modal de filtrado por obra, no hace falta en encargado/oficina no?" — confirmado en `getBobinas()` que el backend ya ignora ese parámetro para cualquiera que no sea superadmin/empresa_admin; se oculta el selector para el resto y se preselecciona con la obra ya elegida para quien sí puede cambiarla.

**v8.35 — aviso de mensajes de chat privados sin leer.** Adrián: "le he mandado un mensaje a alberto pero no lo ve". Verificado en D1 que el mensaje SÍ llegaba bien, a la cuenta activa correcta (ver nota de cuentas duplicadas de Alberto más abajo) — lo que faltaba era cualquier aviso de que había uno nuevo sin tener esa conversación concreta abierta. Nuevo endpoint `/chat/no-leidos` (cuenta mensajes con `destinatario_id` = yo desde una fecha; sin tabla de estado de lectura, la fecha la guarda el cliente en localStorage). Poll cada 20s, badge rojo + parpadeo en el FAB y en el sidebar, pitido corto solo cuando sube el contador.

**v8.36 — el chat DevTools IA se muda del botón flotante al sidebar.** Adrián: "yo sigo viendo dos botones flotantes para alejandra IA" / "las dos burbujas iguales". No era un bug nuevo: `#aiFloatBtn` (chat DevTools, solo desarrollador/superadmin) y `#fabAlejandra` (chat normal) son a propósito dos chats distintos, ya separados desde antes de esta sesión (ver commit `0d246d0`, SCAN-04) — pero apilados en la misma esquina seguían pareciendo iguales. Tras barajar barra superior, se decide por sidebar: nuevo `#btnDevAI` en la sección "Desarrollador" (antes de "DevTools"), `#aiFloatBtn` eliminado del stack flotante. Con eso fuera, desarrollador queda con el mismo par de flotantes (Alejandra + Chat equipo) que cualquier otro rol.

**Deploy:** worker.js desplegado 2 veces en esta parte (tras `/historial` con `obra_nombre` + tras `/chat/no-leidos`) vía `npx wrangler deploy`. Las 9 versiones de frontend (v8.28→v8.36) comiteadas y pusheadas individualmente, cada una verificada en `https://padilla585projects.github.io/Alejandra-APP/panel.html` con el navegador (sesión simulada, nunca con credenciales reales de Alberto/Adrián) antes de darla por cerrada.

**v8.37 — sincronización en tiempo real de inventarios en la app móvil.** Adrián: "continúa con los pendientes" → primero de la lista, "revisa que todos los cambios de inventarios se hagan en tiempo real entre app y office". Investigado: Office ya tenía auto-refresh cada 60s para Bobinas/PEMP/Carretillas desde antes de esta sesión (`SYNC_INTERVALS` en panel.html, sistema `_startSync`/`_stopSync` con chip "⟳ hace Xs" visible en el topbar) — funcionaba bien, no había bug ahí. La app móvil sí tenía un gap real: la pantalla de inventario (`#screenModule`, compartida por los 3 módulos) nunca estaba registrada en la infraestructura de polling genérica que ya usan Pedidos/Incidencias/Personal (`_APP_SCREEN_REFRESH`/`_APP_SECTION_POLL_SECS`/`_startAppSectionPoll`) — solo se refrescaba al entrar en la pestaña o pulsar el botón manual. Se añade con el mismo intervalo (60s), solo mientras el sub-tab "stock" está activo (nunca durante "scan", para no interrumpir cámara/formulario a medio rellenar) — y se descubrió que había que llamar a `_startAppSectionPoll('module')` explícitamente dentro de `navTo()`, porque ese destino gestiona `#screenModule` a mano sin pasar por `_applyScreen()` (que es donde normalmente se dispara el polling para el resto de pantallas).

**v8.38 — chat privado 1 a 1 + aviso de no-leídos en la app móvil (paridad completa con Office).** Último punto grande de la checklist: "todavía no tiene selector de contacto para chat privado 1 a 1 — solo se construyó en Office" (Part 57/58). Se replica el patrón de dos vistas de panel.html (lista de contactos ↔ hilo activo) en `#chatPanel`: nuevo `#chatContactosView`/`#chatHiloView`, botón "←" para volver, `_chatCon` (null=lista / 'todos'=canal equipo / id=privado), `chatMostrarContactos()`/`chatCargarContactos()`/`chatSeleccionarContacto()`/`chatVolverContactos()`. El filtro "Mi obra/General" (que ya existía, propio de la app) solo se muestra en el canal "Todos" — un privado no depende de la obra. `chatCargar()`/`chatPoll()`/`chatEnviar()` adaptados para mandar `con`/`destinatario_id`. De paso, el aviso de mensajes privados sin leer (`/chat/no-leidos`, CHAT-NOTIF-01) que solo existía en Office se añade también aquí (`_checkChatBadgePrivado()`, combinado con el aviso del canal "Todos" que ya existía) — misma precaución que en Office de sembrar la baseline de lectura con el reloj del propio D1 (`ahora`) en vez de `new Date()`, para no desajustar el formato de fecha que compara el backend. Verificado en navegador con sesión simulada: lista de contactos con datos reales, cambio de vista lista↔"Todos"↔privado con Alberto Martínez (id 46, la cuenta real activa) funcionando, filtro de obra oculto correctamente en privado.

**v8.39 — `#remoteScanFab` también movible.** Se había dejado fuera de SCAN-04b (Part 57) por miedo a que arrastrar el contenedor rompiera el click de sus sub-botones (pendientes/escanear). Revisado `makeDraggable()`: solo suprime el siguiente click cuando el puntero se desplaza más de 4px durante el drag — un click normal sin arrastre nunca activa esa rama, así que llega intacto al botón hijo. Seguro aplicarlo igual que al resto de FABs.

**v8.40 — Ingeniería alineada al patrón rol+departamento (INGENIERIA-02).** Última pregunta de la checklist: Adrián eligió "alinear al patrón rol+departamento" (frente a dejarlo como estaba). Antes vivía en `organizarSidebarIngenieria()`, gateada solo por `SESSION.departamento`, con el mismo menú completo para cualquier rol (un operario veía lo mismo que un encargado). Se integra en `_MENU_ROL_DEPT_CONFIG` como un departamento más — pero como Ingeniería no encaja en la forma "Material y maquinaria"/"Documentación y control" del resto (no tiene inventario; sus páginas van en una sección propia, no en 'obra'), `_organizarSidebarRolDept()` gana tres claves condicionales (`hoyEnObra`/`material`/`controlObra`, `false` u omitidas = no tocar esa sección) y una nueva `seccionExtra` genérica (sección + etiqueta + lista de páginas a medida) — sin cambiar el comportamiento por defecto de Eléctrico/Mecánicas/Seguridad, que no necesitaron tocar sus entradas. Se añaden `encargado.ingenieria` e `ingeniero.ingenieria` (rol real y distinto de encargado, confirmado en el `<select>` de roles del panel — es quien de verdad usa este departamento) con la lista íntegra, y `operario.ingenieria` con un subconjunto reducido de ejecución de campo (sin contratos/compras/riesgos/actas — asunción razonable, no confirmada campo a campo, mismo criterio que el resto de subconjuntos "operario" del archivo). `organizarSidebarIngenieria()` se conserva como red de seguridad solo para roles sin entrada propia (p.ej. jefe_de_obra), verificado en navegador que sigue funcionando. También verificado en navegador: rol `ingeniero` reorganiza correctamente ("📐 Ingeniería", inventario oculto, tareas movida a la sección).

**Pruebas en vivo en un HTC real por ADB (continuación de Part 58).** Tras cerrar la checklist en v8.40, Adrián pidió comprobar SEG-02 él mismo como superadmin: "conecta adb" — con dos límites claros mantenidos toda la ronda: Claude nunca introduce credenciales/códigos de acceso (eso lo hace Adrián en el propio dispositivo) y nunca borra datos permanentemente. Con esas reglas, se usó `adb` (WiFi, más estable que USB tras varias desconexiones) para navegar y hacer capturas de pantalla reales del HTC mientras Adrián probaba.

**v8.41 — SEG-02 permite elegir foto de galería, no solo hacer foto.** Adrián probando en vivo: "también podemos agregar imagen que tengamos en el móvil no solo hacer foto". Los inputs de foto (alta de registro + añadir foto al detalle) usaban `capture="environment"`, que fuerza la cámara y no deja elegir de galería — se quita en los 2 inputs de SEG-02 (sin tocar otros que sí necesitan cámara en vivo a propósito, como el escaneo de albaranes).

**v8.42 — barra superior móvil demasiado apretada.** Captura real del HTC: "apenas se ven bien los iconos o letras". El header ya usaba `clamp()` pero con 8 elementos en una fila no había hueco real que ganar. Se libera espacio quitando el emoji 👤 redundante del chip de usuario y achicando el punto de conexión (decorativo), usando ese hueco para subir tamaños de letra/iconos que sí hay que leer.

**v8.43 — nombre de obra autoajustable en vez de truncar.** "Podemos hacerla flexible y autoajustable, así el nombre de obra se ajusta solo". Nueva `_autoFitTexto()`: reduce el tamaño de letra en pasos de 0.5px hasta que el texto quepa entero, en vez de truncar siempre al mismo tamaño fijo. Aplica al texto normal (todos los roles salvo superadmin, que usa un `<select>` nativo no auto-ajustable por JS).

**v8.44 — franja naranja de la barra de estado.** Captura real: "¿por qué se ve la franja naranja arriba? ¿no debería esconderse?". `theme-color`/`theme_color` estaban en `#f97316` (naranja), pintando la barra de estado de Android — se cambia a `#162032` (el mismo navy de la cabecera) para que combine, confirmado en captura real que ya no se ve la franja.

**v8.45 — quita el chip "SA", el logo marca admin/dev; el `<select>` de obra de superadmin seguía truncando.** "Quita el icono de SA y podemos cambiar el icono de la app para diferenciar el acceso admin o desarrollador así nos ahorramos un hueco". Se quita `#headerUserChip`; el logo del header lleva ahora un anillo de color (naranja superadmin, morado desarrollador) vía CSS, con tooltip. De paso: "sale sí, pero mal" — el `<select>` nativo de superadmin seguía truncando ("CPD Ge...") pese a más espacio libre (v8.43 solo le daba más hueco, no bastaba — un `<select>` no permite auto-ajustar el tamaño de letra de la opción elegida por JS). Se envuelve en `headerObraSelectWrap`: el `<select>` real queda invisible pero clicable encima (mismo comportamiento de siempre al tocar), y debajo se ve un span normal sincronizado a mano que sí admite `_autoFitTexto()`. Verificado en captura real del HTC: "CPD Getafe ▾" completo, sin cortar.

**v8.46 — bug real: "Chat del equipo" montado encima de Obra Civil/Pintura.** Adrián: "mira un bug" / "as visto la tarjeta de chat interno montado encima de las demás". Reproducido en navegador simulando el catálogo de departamentos extra cargado (Obra Civil, Pintura, Carpintería...): `.dept-grid` tenía `height:100%`, fijando su caja a la altura de una pantalla; con más departamentos cargados el contenido real es más alto y se desborda SIN mover la caja del grid — el siguiente hermano en el HTML (la tarjeta "Chat del equipo") se posicionaba donde terminaba esa caja de 100%, no donde terminaba el contenido real, montándose encima de las tarjetas desbordadas. `#appContent` ya tiene `overflow-y:auto` y hace de scroll único de toda la pantalla, así que el grid solo necesitaba crecer con su contenido. Verificado con mediciones reales de `getBoundingClientRect()` antes (`overlap: true`) y después (`overlap: false`) del fix.

**v8.47 — DEPT-CONSTR-03: los 6 departamentos que faltaban ya tienen menú propio.** Adrián: "quiero terminar con los roles por departamentos". Antes de construir, pregunta clave sobre Obra Civil (el único de los 6 que ya tenía algo — nunca se le ocultaba la sección Construcción): ¿igual que Mecánicas (material) o con acceso a coordinación técnica? Respuesta de Adrián, más amplia de lo preguntado: **"Albañilería, Pintura, Carpintería se mete dentro de Construcción. Telecom y Almacén departamentos aparte."** Se implementa exactamente así — `_MENU_ROL_DEPT_CONFIG` gana:
- `obra_civil`/`albanileria`/`pintura`/`carpinteria` (encargado + operario): mismo patrón que `ingenieria` (`seccionExtra` sobre la sección 'construccion', sin `material`/`controlObra`/`hoyEnObra`), con icono y etiqueta propios pero **la misma lista de páginas para los 4** — un subconjunto de "cuadrilla de obra" (tareas, RFIs, calidad, punch list, actas, planos, cronograma, control de calidad, ITP, plan semanal, instrucciones), sin las páginas más administrativas/contractuales (contratos, compras, submittals, transmittals, riesgos) que se quedan reservadas para Ingeniería. Operario recibe el mismo subconjunto reducido que ya usaba `ingenieria.operario`.
- `telecom`/`almacen` (encargado + operario): mismo patrón que `mecanicas` (`material`, sin bobinas — exclusiva de Eléctrico). Almacén con una lista algo más corta (sin mantenimientos/repostajes, no son quienes mantienen los vehículos) reflejando su foco en logística/recepción/stock.

De paso, `ocultarSeccionConstruccionSiNoAplica()` se invierte: antes era "ocultar solo para electrico/mecanicas" (todos los demás la veían, curados o no); ahora es una lista explícita de quién SÍ la ve (`obra_civil`, `albanileria`, `pintura`, `carpinteria`, `seguridad`, `ingenieria`), ocultándola también para Telecom/Almacén — antes la veían sin querer, sin estar curada para ellos (no un bug con síntomas, simplemente destino no filtrado).

Ninguno de los 6 tiene cuentas reales todavía (confirmado por Adrián) — igual que el resto de subconjuntos "operario"/departamento nuevo de este archivo, es una asunción razonable sobre qué necesita cada gremio, flaggeada para revisar en cuanto haya un usuario real. Verificado en navegador: configs devuelven la estructura esperada, `ocultarSeccionConstruccionSiNoAplica('telecom', false)` oculta la sección, `organizarSidebarEncargado()` renombra "Material y maquinaria" con el icono de Telecom/Almacén y mueve `herramientas` correctamente (una discrepancia de visibilidad de la cabecera de sección resultó ser una limitación de la prueba aislada — reproducible igual en Eléctrico, departamento ya funcionando en producción — no un bug real).
...
### Part 56: ORG-05 — Menú de Office para Operario en Eléctrico y Mecánicas (25/07/2026) [COMPLETADO, v8.23]

**Contexto:** tras verificar producción con Alberto/Katherine/Adrián, Adrián: "nos queda pendiente seguir con los roles". Repasado el estado (Eléctrico/Mecánicas/Seguridad con Encargado curado, Seguridad con ambos roles, resto de departamentos y Operario en Eléctrico/Mecánicas aún sin menú propio), confirmado por AskUserQuestion: menú de Operario en Eléctrico y Mecánicas primero.

**Diseño:** mismo bloque de "Material y maquinaria" que su Encargado correspondiente (un operario de campo no necesita ver menos material del que maneja día a día, solo menos gestión/administración — la reducción de accesos de gestión ya la aplica `ocultarItemsPorOficio()`/ORG-01 por igual a encargado y operario). A diferencia de Recursos Preventivos en Seguridad (que solo oculta Carnets/Reconocimientos), aquí se oculta TODO lo marcado como sensible (`.nav-sensitive-data` sin acotar), incluida la lista general de "Trabajadores" — mismo criterio que ya usa el panel para el rol Oficina sin admin. Asunción razonable por privacidad, no confirmada campo a campo con Adrián.

**Cambios en `panel.html`:** `_MENU_ROL_DEPT_CONFIG.operario` gana las claves `electrico` y `mecanicas` (mismos iconos y listas de material que `encargado.electrico`/`encargado.mecanicas`, con `ocultarSensibles: true`). Nuevo flag `ocultarSensibles` en `_organizarSidebarRolDept()`, distinto de `ocultarSensiblesSeguridad` (que solo acota a Carnets/Reconocimientos): usa el selector `.nav-sensitive-data` sin combinar con `.nav-seguridad-section`, así que también oculta "Trabajadores". No se tocó la rama de Recursos Preventivos, que sigue igual que en Part 51.

**Verificación:** probado en navegador replicando el orden de carga real: Operario Eléctrico conserva bobinas y la etiqueta ⚡, Operario Mecánicas la oculta con 🔧, ambos sin Trabajadores/Carnets/Reconocimientos visibles. Regresión comprobada: Encargado Eléctrico sigue viendo los tres ítems sensibles sin cambios (su config no lleva `ocultarSensibles`). Cero errores de consola. Solo `panel.html` — sin cambios en `worker.js`, no requiere `wrangler deploy`.
...
### Part 57: Pruebas en vivo con Alberto — bobinas y chat interno rotos (25/07/2026) [COMPLETADO, v8.24→v8.27]

**Contexto:** Adrián y Alberto probando Alejandra Office en producción, en directo, metiendo bobinas. Reportaron problemas en tiempo real, uno detrás de otro, sin pausa — se investigó y arregló cada uno según llegaba, desplegando varias veces durante la sesión (v8.24, v8.25, v8.26, v8.27).

**SCAN-01 (crítico) — devolución de bobinas/PEMP/carretillas rota de raíz:** `devolverBobina/Pemp/Carretilla` en `worker.js` referenciaban la columna `devuelto_por`, que **no existía en ninguna de las 3 tablas** en producción — todo intento de marcar como devuelta fallaba con error SQL 500, tanto desde la app móvil como desde Office. Confirmado con `PRAGMA table_info` en D1 real. Añadida la columna en las 3 tablas.

De paso, auditando las 3 funciones: **ninguna comprobaba autenticación** en la rama "el ítem ya existe" (solo la de auto-creación llamaba `getAuth`), y **ningún SELECT/UPDATE filtraba por `empresa_id`** — cualquier request sin token podía devolver la bobina de OTRA empresa con solo saber/adivinar el código (confirmado con un curl anónimo real contra producción, que efectivamente pudo crear+devolver una bobina de prueba). Corregido: `getAuth()` al principio de las 3 funciones, `empresa_id` exigido y usado en todos los `SELECT`/`UPDATE`. Auditando el resto de endpoints de bobinas se encontró el mismo agujero en `transferirRecurso()` — corregido también.

**SCAN-02 — longitud de bobina nunca se guardaba:** `editarBobina()` no tocaba la columna `longitud` en su `UPDATE` (aunque se llamara con el nombre correcto), y `crearBobina()` tampoco la guardaba al crear. El panel de Office además llamaba a campos `longitud_actual`/`longitud_inicial`, que nunca existieron en la API real (solo hay una columna `longitud`) — las columnas de longitud salían siempre en blanco y "editar" no guardaba nada aunque el toast dijera "Guardado ✓". El editor de Estado inline además ofrecía valores (disponible/asignada/agotada) que no son los reales que usa el resto de la app (`activa`/`devuelta`, confirmado con `SELECT estado, COUNT(*) GROUP BY estado` en producción).

**Pedido explícito de Adrián: "quiero un modal en el inventario cada bobina para cambiarla de estado".** Sustituidos los editores de celda rotos por un modal de gestión por bobina (longitud + estado real, con flujo correcto de devolución vía el endpoint dedicado cuando se marca "devuelta" — pide Devuelto por + notas). Formulario de alta completado con nº de albarán y notas (la app móvil ya los pedía).

**SCAN-03 — fechas "Invalid Date" en todo Office:** `fechaEspana()` (worker.js) guarda con `new Date().toLocaleString('es-ES', {...})`, que produce texto tipo "25/7/2026, 11:22:29" (formato español, no ISO). `new Date()` de `fmtFecha()`/`fmtHora()` no lo parseaba de forma fiable → "Invalid Date" en toda fecha guardada por ese helper (fecha_entrada, fecha_devolución… de bobinas/PEMP/carretillas y más). Se añadió un parseo manual (`_parseFechaBackend`) del formato español antes de caer a `new Date()`. Parche en el formateador, no en el origen — ver pendientes.

**SCAN-04/04b — botones flotantes de Alejandra solapados y ahora movibles:** `#fabAlejandra` (chat normal, cualquier sesión) y `#aiFloatBtn` (chat DevTools IA, solo desarrollador/superadmin) estaban casi en la misma esquina — para una cuenta desarrollador (Adrián), que ve los dos, se tapaban. Apilados verticalmente (mismo patrón que `#remoteScanFab` ya usa sobre `#fabAlejandra`). Después, a petición de Adrián, se hicieron arrastrables con el ratón/dedo (posición recordada por navegador vía `localStorage`) — `#remoteScanFab` se dejó fuera a propósito por ser un contenedor con varios sub-botones.

**SCAN-05 — chat interno mostrando mensajes nuevos arriba:** `getChatMensajes()` en el backend ya devuelve los mensajes en orden cronológico (más antiguos primero) en las dos rutas (carga inicial y polling incremental), pero `cargarChat()` en `panel.html` aplicaba un `.reverse()` extra que los volvía a invertir. Quitado.

**CHAT-01 (el hallazgo más grande) — el chat nunca fue un chat de verdad:** al investigar por qué Adrián no veía nombres de otras personas escribiendo, se descubrió que `getChatMensajes()` filtraba siempre por **un solo `usuario_id`** (por defecto, el propio si no se pasaba parámetro) — cada usuario **solo veía sus propios mensajes**, nunca los de nadie más. Explicaba de golpe tres preguntas seguidas de Adrián: por qué nunca veía nombres ajenos, cómo comunicarse con Alberto (no había forma), y por qué no podía elegir destinatario (no existía). Confirmado por AskUserQuestion: modelo privado 1 a 1 + canal "Todos", con selector de contacto; notificaciones push confirmadas como deseadas pero pospuestas ("las dos, primero que funcione").

Reescrito: nueva columna `destinatario_id` en `chat_mensajes` (NULL = canal "Todos", un id = mensaje privado). `getChatMensajes` acepta un parámetro `con` (ausente o `'todos'` = canal de equipo; un id = hilo privado con esa persona, mensajes en ambas direcciones). `enviarChatMensaje` acepta `destinatario_id`. En `panel.html`, el panel de chat pasa a tener dos vistas: lista de contactos (reutiliza `/personal/trabajadores`, sin necesitar permisos de admin) y el hilo activo, con botón para volver a la lista. La app móvil no enviaba `usuario_id`/`con` en ninguna petición, así que cae automáticamente al canal "Todos" con el nuevo backend — sin romperse, pero sin selector de contacto propio todavía (pendiente).

**SCAN-06 — columna de nº de albarán ausente:** ya se pedía en el alta y la API ya la devolvía, pero no tenía columna en la tabla de Bobinas de Office. Añadida, y el buscador ahora también filtra por ella.

**Verificación:** cada fix se probó de forma aislada (curl directo contra producción para los endpoints, `node --check`/bloques `<script>` para sintaxis, sin funciones duplicadas) y se desplegó de inmediato (`npx wrangler deploy` para el backend en cada cambio de `worker.js`, commit+push para que GitHub Pages sirviera el frontend — en un punto se detectó que unos cambios de frontend se habían escrito pero no subido, dejando producción sirviendo código viejo un rato; corregido subiendo enseguida). Sin corrupción de encoding en ningún commit. Versiones sincronizadas en cada subida (v8.24 → v8.27).

**Sin cambios en `alejandra-agente/worker.js`:** ninguno de estos fixes toca la barrera destructiva, tools ni permisos del agente — son bugs de datos/UI/chat del worker principal.

### Part 54: DATA-KATHERINE — Cuentas duplicadas de Katherine unificadas (25/07/2026) [COMPLETADO, solo datos]

**Contexto:** Adrián pidió verificar SEG-02 con la cuenta real de Katherine. Auditoría en D1 producción: 4 filas con nombre "Katherine" (ids 8, 39, 42, 45). Las ids 39 y 42 ya estaban inactivas y sin ningún dato asociado (sesiones, fichajes, sync_dispositivos, carnets, epis_asignados, turnos, chat_mensajes) — no se tocaron.

**La contradicción:** la id 8 (`katherine@test.local`) tenía actividad real reciente (sesión y mensaje de chat del 18/07/2026) pero un código `_del_8` — la misma marca que usa el propio endpoint de borrado de usuarios (`codigo='_del_'||id`, `worker.js:7043`/`7047`), que también debería poner `activo=0` y vaciar email/contraseña. En la 8 ninguna de esas dos cosas se había hecho: seguía activa, con email de prueba y contraseña. La id 45 (Katherine El Souki, `katherineelsouki@gmail.com`) tenía el nombre y el email reales, pero sin contraseña y con un código `g_pend_...` de alta con Google aparentemente sin terminar — y su última sesión (13/07) era más antigua que la de la 8.

Revisando el flujo de aprobación de altas por Google (`worker.js:3837`), se confirmó que la 45 SÍ estaba aprobada de verdad (`activo=1`, rol y departamento correctos); el código `g_pend_` es solo un resto cosmético que ese endpoint no limpia, y al ser cuenta de Google no necesita contraseña para entrar. Aun así, la contradicción (actividad real en la cuenta de pruebas vs. datos reales en la cuenta con alta a medias) no se podía resolver solo mirando la base de datos — se preguntó a Adrián por AskUserQuestion cuál era la cuenta real. Confirmó: la 45.

**Cambios aplicados en D1** (resultados capturados en la sesión antes de tocar nada, mismo patrón que DATA-ALBERTO): `DELETE` de la sesión activa de la 8 (id 152, última actividad 18/07), y completado su borrado con el mismo `UPDATE` que usa el endpoint real de borrado (`activo=0, email=NULL, password_hash=NULL`; `codigo` se queda `_del_8`, ya coincidía). La 45 no necesitó ningún cambio — ya estaba correctamente configurada y funcional.

**Sin cambios de código en ningún archivo:** limpieza de datos pura, no toca los dos cerebros de Alejandra, no requiere deploy ni subida de versión.

### Part 53: SEG-02 — Registro de Seguridad: fotos + comentarios del departamento + informe (25/07/2026) [COMPLETADO, v8.22]

**Contexto:** Adrián, tras cerrar el menú de Seguridad (Part 51): "en seguridad deberiamos de tener una funcion solo para subir fotos de incidencias o estados de la obra donde luego se puedan verificar desde la app y el office por cualquiera del departamento de seguridad. donde se pueda hacer comentarios". Ampliado después: "incluso una opcion aparte de generacios de informe añadiendo fotos y explicando el incidente o lo que sea incluso mandarlo por mail despues o compartirlo por whatsap, descargarlo...lo que sea. vamos lo que hace un departamento de seguridad".

**Investigación previa (para no duplicar):** ninguna funcionalidad existente cubría esto. Galería (`fotos_obra`) admite un único comentario fijo por foto, no un hilo. Incidencias admite varias fotos pero solo descripción inicial, sin hilo de comentarios, y su aislamiento por departamento ya impediría que Seguridad viera incidencias de otros departamentos salvo con rol privilegiado. Los módulos propios de Seguridad (EPIs, carnets, reconocimientos, permisos de trabajo, ATS, accidentes) no admiten fotos ni comentarios. No existía ningún patrón de "comentarios múltiples de varias personas sobre un registro" en todo el proyecto — lo más cercano, `chat_mensajes`, no tiene FK a ningún registro. Confirmado por AskUserQuestion: módulo nuevo e independiente (no dentro de Incidencias), visible y comentable por todo el departamento de Seguridad + admins, informe en PDF vía ventana de impresión (mismo patrón que Partes de trabajo/Informe de obra).

**Cambios en `worker.js`:**
- 3 tablas nuevas (migración perezosa `ensureSegRegistrosTables`): `seg_registros` (id, empresa_id, obra_id, titulo, descripcion, creado_por, created_at), `seg_registro_fotos` (FK `registro_id`, mismo patrón que `incidencia_fotos`), `seg_registro_comentarios` (FK `registro_id`, mismo patrón de autoría que `chat_mensajes`: usuario_id, usuario_nombre, mensaje, created_at).
- Helper `puedeVerSegRegistros(auth)`: mismo criterio que el Inventario de EPIs (DEPT-02) — superadmin/empresa_admin/desarrollador/Seguridad. A propósito **sin** `isProjectManager`: la vista ampliada de Project Manager (ROLES-03) solo se pidió para el inventario, no para este módulo nuevo.
- 8 endpoints: `GET/POST /seg-registros`, `GET/DELETE /seg-registros/:id`, `POST /seg-registros/:id/fotos` (multipart, mismo patrón que `subirFotoIncidencia`: máx 20MB, solo imágenes), `GET/DELETE /seg-registro-fotos/:id` (sirve desde R2), `POST /seg-registros/:id/comentarios`, `POST /seg-registros/:id/email` (reutiliza `enviarEmailResend`, mismo patrón que `enviarPedidoPorEmail` de Part 38 — mismo aviso pendiente: el dominio sandbox de Resend puede no entregar a destinatarios externos).
- **Decisión de seguridad explícita:** el email NO incluye las fotos embebidas. `/seg-registro-fotos/:id` exige sesión autenticada, y no hay en todo el proyecto un precedente de exponer una imagen protegida por token dentro de un email externo (riesgo de fuga del token de sesión al destinatario). El informe con fotos vive en el PDF, generado dentro de la sesión autenticada del usuario.

**Cambios en `panel.html`:**
- Nueva página "📷 Registro de Seguridad" dentro de la sección lateral "🔺 Seguridad": lista de tarjetas (título, obra, extracto, nº fotos/comentarios) + modal de alta + modal de detalle (fotos, descripción, hilo de comentarios con input de envío, y botones Imprimir/Email/Compartir/Borrar).
- `segRegImprimir()`: ventana de impresión con las fotos (vía URL con token), descripción y el hilo completo de comentarios — mismo patrón que `parteImprimir`/`generarInformeObra`.
- `segRegCompartir()`: `navigator.share()` con fallback a enlace `wa.me` — mismo patrón ya usado en `index.html` para compartir el resumen de stock.
- **Bug lateral encontrado y arreglado (no dejado para después):** la sección "🔺 Seguridad" del sidebar solo se revelaba si `SESSION.departamento === 'seguridad'` — **ni siquiera un admin la veía** si su departamento activo era otro, inconsistente con "🔺 Bobinas"/`.nav-electrico-section` (INV-01), que sí incluye `isAdmin`. Sin este fix, el nuevo módulo (confirmado como "Seguridad + admins") no habría llegado realmente a los admins. Corregido añadiendo `isAdmin ||` a la condición, alineado con el precedente ya existente.

**Cambios en `index.html`:**
- Nueva tarjeta "Registro de Seguridad" y nueva pestaña dentro de la pantalla existente de Seguridad (`segSelTab('registro')`), con captura de foto por cámara (`capture="environment"`), lista, detalle con fotos+comentarios, y añadir foto/comentario adicionales. Sin impresión/email/compartir en móvil a propósito — ese informe completo vive en Office; el móvil es para capturar rápido en el momento.

**Sin cambios en `alejandra-agente/worker.js`:** no es una barrera destructiva, tools ni permisos del agente — es un módulo de negocio del worker principal.

**Verificación:** `node --check worker.js` OK; 3+3 bloques `<script>` sin errores; sin funciones duplicadas (comprobado explícitamente, patrón BOTONES-01); sin corrupción de encoding; versiones sincronizadas v8.22; `npx wrangler deploy` con bindings D1/R2 OK. En producción, los 8 endpoints nuevos responden 403 sin credenciales (confirmado que las rutas existen y están protegidas, tras un falso positivo inicial por un artefacto del script de prueba con `curl -X GET -d`). Probado en navegador real con datos simulados: en Office, un admin con departamento activo `electrico` ve el nuevo ítem del sidebar (confirma el fix del bug lateral), el modal de detalle muestra título/descripción/comentario/autor correctamente, y un comentario nuevo se envía con el payload esperado. En la app móvil, la pestaña carga la lista con contadores de fotos/comentarios, abre el detalle con la descripción y el comentario existente, y un comentario nuevo se envía correctamente. Cero errores de consola en ambas pruebas.

### Part 52: ROLES-04 — Nombres por departamento (Técnico/Recursos Preventivos) + catálogo de roles completo (25/07/2026) [COMPLETADO, v8.21]

**Contexto:** al construir el menú de Seguridad (Part 51), Adrián precisó la terminología real de la empresa: en Seguridad, Encargado se llama **Técnico** y Operario se llama **Recursos Preventivos** (RP) — ejemplo dado: "Katherine en seguridad sería un Recurso Preventivo (RP)". Confirmado explícitamente vía AskUserQuestion que es el **mismo rol interno** (`encargado`/`operario`, mismos permisos en worker.js), solo cambia el nombre que ve el usuario — coherente con "cada departamento tiene los mismos roles de Eléctricos".

**Cambios en `panel.html`:**
- Nuevo helper `rolLabelPorDepto(rol, departamento)` con `_ROL_LABEL_BASE` (catálogo completo: incluye `ingeniero`, que faltaba) y `_ROL_LABEL_POR_DEPTO` (`seguridad: { encargado: 'Técnico', operario: 'Recursos Preventivos' }`). Sustituye al objeto `rolLabels` fijo del topbar y se aplica también en la lista de invitaciones, el badge de Rol y de Roles extra de la tabla Usuarios, y el modal de roles extra — en todos esos sitios ya había disponible el `departamento` de la fila/sesión, así que no hizo falta pasar datos nuevos.
- **Bug lateral confirmado y arreglado (no dejado para después):** al revisar los selectores de rol se encontró que `project_manager` y `almacenero` (roles reales desde ROLES-02, v8.17) **faltaban en 5 de los 7 sitios** donde se elige o filtra un rol: el editor de Rol de la tabla Usuarios, el modal "Nuevo usuario", el modal "Nuevo trabajador" y el filtro de la tabla Usuarios. Solo el selector de roles extra y el formulario de invitaciones ya los tenían. Añadidos en los 6 sitios afectados, junto con el nuevo rol `ingeniero`.
- **Bug lateral #2, más serio:** el modal "Nuevo trabajador" (`abrirModalPersonal()`) tenía el desplegable de Departamento **escrito a mano** con `<option value="oficina">Oficina técnica</option>` — ese departamento ya no existe desde INGENIERIA-01 (v8.19), que lo sustituyó por `ingenieria` (confirmado contra `validos[]` en worker.js: 11 departamentos reales, ninguno llamado `oficina`). El desplegable tampoco ofrecía `ingenieria`. Cualquier alta de trabajador desde ese modal con "Oficina técnica" habría guardado un departamento inválido, y era imposible dar de alta a alguien directamente en Ingeniería desde ahí. Sustituido por el catálogo real `_DEPTS_CATALOG`/`_ALL_DEPTS`, la misma fuente que ya usaba correctamente `abrirModalUsuario()` — quedan sincronizados para siempre por construcción, no habrá que tocar dos sitios la próxima vez que se añada un departamento.
- Rol `ingeniero` dado de alta: añadido a los 7 selectores de rol/filtro, con color de badge propio (`#ec4899`), acceso a Office igual que Encargado (necesita el rol acumulado `oficina`, sin cambios en `puedeEntrarOffice()` — ya funciona para cualquier rol con `oficina` en `roles_extra`, no hizo falta tocarlo). El menú de Ingeniería (`organizarSidebarIngenieria()`, INGENIERIA-01) sigue gateado por departamento, no por rol — no necesitaba cambios: cualquier cuenta con `departamento==='ingenieria'` ya ve ese menú, sea cual sea su rol.

**Verificación:** `node --check worker.js` OK (sin cambios); 3+3 bloques `<script>` sin errores; sin corrupción de encoding. Probado en navegador real: `rolLabelPorDepto('encargado','seguridad')` → "Técnico", `rolLabelPorDepto('operario','seguridad')` → "Recursos Preventivos"; el modal "Nuevo trabajador" ya ofrece los 11 departamentos reales (incluida Ingeniería, sin "Oficina técnica"); una fila simulada de Katherine (`operario`/`seguridad`) en la tabla Usuarios renderiza el badge "Recursos Preventivos". Cero errores de consola.

**Sin cambios en `alejandra-agente/worker.js` ni en `worker.js`:** es una capa de nombres visibles y selectores de UI, ningún permiso de backend cambia.

### Part 51: Menú de Office para Seguridad — Técnico y Recursos Preventivos (25/07/2026) [COMPLETADO, v8.21]

**Contexto:** siguiente departamento tras Mecánicas (Part 50), confirmado por AskUserQuestion. A diferencia de Eléctrico/Mecánicas, Seguridad ya tenía una sección propia en el sidebar ("🔺 Seguridad": EPIs, Carnets, Reconocimientos, Permisos de Trabajo, ATS, Accidentes/Incidentes), revelada por departamento en `iniciarApp()` — no hacía falta moverla, solo curar el resto del sidebar alrededor y no ocultarla.

**Cambios en `panel.html`:**
- `_MENU_ROL_DEPT_CONFIG` (antes `_ENCARGADO_DEPT_CONFIG`, renombrado y generalizado) pasa a tener una clave por **rol** (`encargado`, `operario`), cada una con sus departamentos. Antes solo existía la rama `encargado`; `organizarSidebarEncargado()` (nombre conservado por sus dos llamadas ya existentes desde `iniciarApp()`/`ocultarSidebarPorPermisos()`) ahora resuelve por `SESSION.rol` **y** `SESSION.departamento` a la vez, delegando en `_organizarSidebarRolDept()` (antes `_organizarSidebarEncargadoDept`).
- `encargado.seguridad`: icono 🔺, bloque "Material y maquinaria" reducido a `herramientas`+`alertasStock` (bobinas/pemp/carretillas no aplican a Seguridad).
- `operario.seguridad` (Recursos Preventivos): mismo bloque de material, más `ocultarSensiblesSeguridad: true` — oculta Carnets y Reconocimientos (clase `nav-sensitive-data`, datos personales/médicos de otros trabajadores), mismo criterio de privacidad que ya aplica el panel al rol `oficina` sin admin. **Asunción no confirmada explícitamente por Adrián** (razonable por privacidad, a revisar si Recursos Preventivos necesita ver esos dos módulos en la práctica).
- Operario en otros departamentos (Eléctrico, Mecánicas) sigue sin menú propio, a propósito — no se pidió y no hay confirmación de contenido para esos casos.

**Verificación:** probado en navegador real simulando la carga real (`.nav-electrico-section`/`.nav-seguridad-section` reveladas por departamento antes de correr la curación, igual que hace `iniciarApp()`): Técnico ve los 6 ítems de la sección Seguridad, Recursos Preventivos ve 4 (sin Carnets ni Reconocimientos), y no hay regresión en Eléctrico (bobinas visible) ni Mecánicas (bobinas oculta). Cero errores. `node --check worker.js` OK (sin cambios en este Part); sin corrupción de encoding.

### Part 50: Menú de Office propio para Mecánicas — ORG-03 (25/07/2026) [COMPLETADO, v8.21]

**Contexto:** Adrián: "ahora tenemos que arreglar los demás departamentos con sus roles que esten bien como hemos echo con Electricas". `organizarSidebarEncargado()` (Part 45/ORG-02, v8.14) solo comprobaba `SESSION.rol==='encargado'`, sin mirar el departamento — un encargado de Mecánicas veía el mismo menú que uno de Eléctrico, con "Bobinas" (cable eléctrico, no aplica a Mecánicas) y la etiqueta "⚡ Material y maquinaria" con el rayo fijo. Confirmado por AskUserQuestion: menú propio por departamento (no genérico por rol), empezando por Mecánicas.

**Cambios en `panel.html`:** lógica común extraída a `_organizarSidebarEncargadoDept(config)` (parametrizada por icono + lista de módulos de "Material y maquinaria"); `organizarSidebarEncargado()` pasa a enrutar por `SESSION.departamento`. Config de Mecánicas: icono 🔧, mismo bloque de material que Eléctrico **sin** `bobinas` (PEMP/Carretillas/Herramientas ya son compartidos entre ambos departamentos en toda la app — `cfgPempSection`/`cfgCarrSection`/`cfgHerrSection` en `index.html`; `bobinas` es exclusivo de Eléctrico). Departamentos aún sin confirmar con cuentas reales (obra_civil, albañilería, pintura, carpintería, telecom, almacén) se dejan sin curar a propósito.

**Verificación:** probado en navegador real replicando el orden de carga real (`iniciarApp()` revela `.nav-electrico-section` solo si `dept==='electrico'`, luego `organizarSidebarEncargado()` cura el resto): Eléctrico conserva bobinas visible y la etiqueta ⚡ sin cambios; Mecánicas oculta bobinas correctamente y usa 🔧. Cero errores de consola. Cambio solo en `panel.html`, sin tocar `worker.js` — no requiere `wrangler deploy`, se publica con el push a GitHub Pages.
...
**Bug reportado (Ausencias):** `abrirModalAus()`/`guardarAus()`/`calcularDiasHabiles()` referenciaban ids de DOM que no existen (`ausModalTrabajador`, etc.) en vez de los reales (`ausNombre`, `ausTipo`, `ausEstado`, `ausFechaIni`, `ausFechaFin`, `ausDiasHab`, `ausAprobador`, `ausFechaApro`, `ausMotivo`, `ausNotas`) — un `TypeError` silencioso al hacer `.value` sobre `null` abortaba la función antes de mostrar el modal, sin ningún error visible para el usuario. Corregidas las 3 funciones para usar los ids reales.

**Auditoría "por si acaso" (panel.html, ~32.000 líneas en un solo archivo):** se hizo un escaneo automático de nombres de función duplicados (`function X` declarada 2 veces al mismo scope global — la última gana, la primera queda muerta sin error) y de ids de DOM duplicados (`getElementById` siempre resuelve al primero en el documento). Encontrados y corregidos 4 casos más de "botón no conectado" del mismo tipo de bug:
- **NCR ("+ Nueva NCR"):** dos implementaciones completas (una embebida en "Calidad", otra en la página standalone "No Conformidades", marcada NEW-72) compartían nombres de función (`abrirModalNcr`/`editarNcr`/`guardarNcr`/`eliminarNcr`) Y el mismo id de modal (`modalNcr`) — ambos botones rotos. Fix: renombrada la implementación más antigua (Calidad) a sufijo `*Qa` (`abrirModalNcrQa`, `modalNcrQa`, etc.), dejando la standalone con los nombres originales. Además, la standalone tenía un bug propio no relacionado: usaba `classList.add('show')` pero el CSS solo reconoce `.modal-overlay.open{display:flex}` — corregido a `'open'`.
- **Catálogo de Precios Unitarios ("+ Nuevo Precio"):** una función incompleta "Cronograma de Pagos" (NEW-115, sin página ni modal HTML en ningún sitio del archivo) reutilizaba los mismos nombres (`abrirModalCp`/`editarCp`/`guardarCp`/`eliminarCp`/`cerrarModalCp`/`modalCp`) que la función SÍ completa y funcional de Catálogo de Precios, tapándola por completo. Fix: renombrada Cronograma de Pagos a sufijo `*Cpg`. Cronograma de Pagos sigue sin funcionar (nunca tuvo HTML propio) pero ya no rompe Catálogo de Precios.
- **Órdenes de Cambio ("+ Nueva OC"):** dos páginas completas con el mismo id `id="pageOrdenesCambio"` y las mismas funciones (`cargarOrdenesCambio`/`abrirModalOC`/`eliminarOC`) — solo la última declarada (y la primera página en el DOM) sobrevive. La implementación ganadora tenía además un bug propio: su `abrirModalOC` llamaba al helper genérico `abrirModal()` con 1 solo argumento (el helper espera 3) y usaba la clase CSS equivocada (`.modal` en vez de `.modal-overlay`) para el modal que construye dinámicamente. Fix: reescrito `abrirModalOC`/`cerrarModalOC` para construir e insertar su propio `.modal-overlay > .modal` autocontenido vía `insertAdjacentHTML`, sin depender del helper genérico. La otra página/implementación (más antigua) queda como código muerto inofensivo, sin tocar (bajo prioridad, ver pendientes).
- **`verPlano` (visor de planos):** dos funciones completamente distintas con el mismo nombre — una para "Planos de Obra" (abre el archivo subido en pestaña nueva) y otra para "Planos IA" (visor SVG interactivo). La segunda (declarada después) ganaba siempre, así que los botones de "Planos de Obra" llamaban al visor SVG equivocado (con id de plano de un namespace distinto) y nunca abrían el archivo real. Fix: renombrada la de "Planos de Obra" a `verPlanoObra` (2 call-sites actualizados), dejando `verPlano` para el visor SVG de "Planos IA".
- **`cargarObrasPanel`:** dos definiciones — la primera (más completa) rellenaba 7 `<select>` de obra en distintas páginas (Bobinas, PEMP, Carretillas, Pedidos, Presupuesto, Fases, Partes) leyendo `/obras`; la segunda (trivial, ganadora) solo hacía `window._obrasPanel = (await api('/mi-empresa')).obras`, dejando esos 7 selects sin rellenar nunca en toda la app. Fix: fusionadas en una sola función (la completa, vía `/obras`) que además asigna `window._obrasPanel` para no romper a los callers que dependían de esa variable global; eliminada la definición duplicada.

**Verificación:** los 3 bloques `<script>` de panel.html compilan sin error (`node -e "new Function(...)"`) tras cada cambio. Live-test con sesión simulada (`localStorage.panel_session` + reload, servidor de preview local puerto 3001): `abrirModalCp()` (Catálogo de Precios) abre el modal real (`display:flex`); `abrirModalCpg()` (Cronograma de Pagos) sigue sin HTML, como ya se sabía, sin regresión; `abrirModalNcrQa()` y `abrirModalNcr()` abren ambos modales reales; `abrirModalOC()` abre el modal con `display:flex` y clase `.modal-overlay` correcta. `git diff` sin corrupción de encoding nueva (`grep "Ã|Â|â€|ï»¿"` sin coincidencias en líneas añadidas).

**Pendiente, detectado pero NO corregido en esta sesión (bajo prioridad, flagged para revisión futura):**
- **Escandallo de Precios** (NEW-114) y **Registro Diario de Prevención** (NEW-116): ambas tienen toda la lógica JS (`cargarEscandalloPrecios`, `cargarRdpRegistros`, etc.) pero CERO página/modal HTML en todo el archivo — navegar a ellas lanzaría un error inmediato (`.innerHTML` sobre `null`). No estaban en el reporte original de Adrian (Personal), no se tocaron.
- **Cronograma de Pagos** (ahora `*Cpg`): sigue sin página/modal HTML propio, solo se evitó que rompiera Catálogo de Precios — la feature en sí sigue sin construir.
- **Órdenes de Cambio, implementación antigua** (HTML ~línea 8173, JS 17872-17983): queda como código muerto (página con id duplicado nunca se muestra, funciones nunca se llaman) — inofensivo pero se podría limpiar en una sesión futura de housekeeping.

**Alcance y despliegue:** solo `panel.html` (frontend estático de GitHub Pages) — no requiere `wrangler deploy`, solo push. Sin cambio de versión de app (panel.html no tiene `APP_VERSION` propio, mismo criterio que sesiones anteriores de solo-panel). No se tocó `worker.js` ni `alejandra-agente/worker.js` — el bug es puramente de marcado/JS del frontend, sin lógica de negocio ni permisos de backend involucrados.

---

### Anterior: Adrian preguntó si de verdad un encargado de un oficio (caso concreto: Alberto) necesita ver tantos botones en el sidebar. Auditoría real: un encargado no-admin veía 62 botones, muchos de gestión de proyecto/PM (RFIs, submittals, presupuestos, contratos, cierre de obra...) que son cosa de jefe_de_obra/oficina, no de un encargado de campo. Adrian además pidió explícitamente que cada encargado/operario pueda ver documentos/planos/fotos de obra **de su propio departamento**. Implementado: (1) nuevo tercer nivel de permisos en el sidebar -antes solo admin/no-admin-: `ocultarItemsPorOficio()` oculta 27 botones de PM/oficina técnica dentro de Obra/Construcción/Planificación SOLO para encargado/operario "puros" (sin roles_extra de jefe_de_obra/oficina/admin); jefe_de_obra/oficina mantienen el set completo que ya tenían. (2) Se movió el botón "Galería de Fotos" (`fotosObra`) de la sección "Diario y Campo" (oculta para no-admin) a la sección "Obra" (siempre visible), porque antes ni encargados ni jefes de obra podían verlo. (3) Backend (`worker.js`): `getPlanosObra` no tenía NINGÚN filtro por departamento (solo por disciplina, texto libre); se añadió `DISCIPLINA_DEPT_MAP` que cruza disciplina↔departamento -electrico, mecanico/fontaneria/climatizacion→mecanicas, civil→obra_civil, seguridad→seguridad- y sin privilegio -admin/desarrollador/Seguridad- solo se ven las disciplinas propias + las transversales sin dueño -arquitectura, estructural, general-. `listarFotosObra` filtraba por departamento SOLO si el frontend lo pedía por query string -cualquiera podía ver fotos de cualquier departamento pidiendo la URL sin ese parámetro-; ahora se fuerza siempre el departamento de la sesión salvo privilegio. `documentosObra` ya estaba bien -filtrado por departamento desde DEPT-01-, no se tocó. Deploy de worker.js hecho y verificado (`/health` 200). Ver Part 27 para detalle completo, incluida la clasificación ítem a ítem de qué se oculta y por qué.
...
### Part 27: ORG-01 — Sidebar organizado de verdad por rol/departamento (22/07/2026)
**Contexto:** Adrian: *"luego quiero que mires alejandra office con la sesion de alberto. dime de verdad si un encargado de electricos necesita ver tantos paneles a la izquierda? revisa eso"*. Alberto es un encargado real de electricidad. Auditoría del código real (no de la documentación) confirmó que un encargado no-admin veía **62 botones** en 6 secciones, y que dentro de "Planificación" (30 ítems) había una mezcla brutal de cosas operativas de campo con herramientas de gestión de proyecto/oficina técnica -RFIs, submittals, transmittals, NCRs, presupuestos SOV, contratos, cierre de obra, certificaciones- que nunca debería necesitar un encargado de un oficio concreto.

**Aclaración de Adrian sobre documentos:** *"El Encargado de cada oficio debe ver documentos de su oficio. en realidad los operarios sea los que sean deben poder ver documentos de obra (planos, esquemas, fotos etc)"* + *"cada uno en su departamento claro"* — es decir, no basta con recortar el ruido de PM, también hay que garantizar que documentos/planos/fotos de obra se vean filtrados por el departamento propio del usuario (no todo, no nada).

**Diagnóstico del backend (antes de tocar nada):**
- `documentosObra` (documentos_obra) ya estaba bien: filtrado por departamento desde DEPT-01 (21/07/2026), visible en lectura para cualquier rol autenticado. No requería cambio.
- `planosObra` (planos_obra) **no tenía columna de departamento**, solo `disciplina` (texto libre elegido al subir el plano: electrico, mecanico, fontaneria, climatizacion, civil, seguridad, arquitectura, estructural, general...). Sin filtro real: cualquiera veía todos los planos de la obra sin importar su departamento.
- `fotosObra` (fotos_obra) sí tenía columna `departamento`, pero el filtro solo se aplicaba si el frontend lo pedía explícitamente por query string (`?departamento=X`) — pidiendo la URL sin ese parámetro, cualquiera veía fotos de todos los departamentos. Además esta página vivía dentro de "Diario y Campo", sección oculta del todo para no-admin, así que ni encargados ni jefes de obra podían verla en absoluto.

**Decisión de diseño (nivel de permisos nuevo):** hasta ahora el sidebar solo distinguía admin / no-admin, tratando a encargado, jefe_de_obra y oficina como un único bloque con el mismo menú. Se introduce un tercer nivel: **encargado/operario "de oficio"** (sin ningún rol de gestión de obra ni admin en `roles_extra`) ven un sidebar recortado; **jefe_de_obra/oficina** mantienen el set completo de siempre (ellos sí gestionan el proyecto).

**Implementación (panel.html):**
- `ITEMS_OCULTOS_OFICIO`: lista de 27 `data-page` que se ocultan solo para encargado/operario puros, clasificados así:
  - Obra (2 de 8 ocultos): `obras` (listado multi-obra, gestión), `documentos` (documentación general de empresa, no de campo). Se quedan visibles: incidencias, pedidos, mantenimientos, documentosObra, inspecciones, fotosObra.
  - Construcción (5 de 8 ocultos): `rfis`, `calidad`, `contactosObra`, `visitasObra`, `actasReunion` (coordinación técnica/contractual). Se quedan: tareas, seguridadObra, punchList.
  - Planificación (20 de 30 ocultos): `presupuesto`, `fases`, `hitos`, `costesObra`, `contratosObra`, `ordenesCompra`, `submittals`, `finanzasObra`, `cronogramaObra`, `transmittals`, `controlCalidad`, `presupuestoSov`, `ordenesCambio`, `riesgosObra`, `obraDashboard`, `informeObra`, `subcontratas`, `registroAmbiental`, `cierreObra`, `certificaciones`. Se quedan: calendario, planSemanal, instruccionesObra, accionItems, centroAlertas, planosObra, entregasMaterial, equiposMedicion, itpObra, ncrsObra.
- `ocultarItemsPorOficio()`: aplica la lista solo si `hasSessionRole('encargado','operario') && !hasSessionRole('superadmin','empresa_admin','desarrollador','jefe_de_obra','oficina')` -para no restringir por error a alguien con doble rol-. Se llama desde `iniciarApp()` (justo tras el bloque de ocultación de secciones) y desde `ocultarSidebarPorPermisos()` (el pase de refuerzo con `setTimeout`), mismo patrón que ya usaba SIDEBAR-01/DEPT-01 para ese tipo de ocultación.
- Reubicado el botón `fotosObra` ("Galería de Fotos"): sacado de la sección "Diario y Campo" (oculta para todo no-admin) y metido en la sección "Obra" (siempre visible para cualquier rol autenticado) — mismos onclick/icono, sin duplicar el botón, verificado que solo existe un `data-page="fotosObra"` en el DOM tras el cambio.
- Resultado para un encargado: de 62 a **~35 botones** (Principal 3, Personal 7, Inventarios 7 -sin cambios en esta sesión, ver pendiente abajo-, Obra 6, Construcción 3, Planificación 10).

**Implementación (worker.js):**
- `DISCIPLINA_DEPT_MAP` (nueva constante, junto a `getPlanosObra`): mapea disciplina→departamento(s) propietario(s) -`electrico`→electrico, `mecanico`/`fontaneria`/`climatizacion`→mecanicas, `civil`→obra_civil, `seguridad`→seguridad-. Las disciplinas sin entrada en el mapa (`arquitectura`, `estructural`, `general`...) se consideran transversales -sin dueño natural, cualquier oficio puede necesitarlas de contexto- y se ven siempre.
- `getPlanosObra`: añadido filtro SQL -sin privilegio (`isDeptPrivileged`: admin/desarrollador/Seguridad)- que solo permite ver planos de disciplinas transversales o de la(s) disciplina(s) que pertenecen al departamento propio de la sesión.
- `listarFotosObra`: antes el filtro de departamento era opcional (solo si `?departamento=` venía en la query string). Ahora, sin privilegio, se fuerza siempre el departamento de la sesión -o fotos sin departamento asignado, que se consideran compartidas-, ignorando lo que pida la query string; con privilegio se sigue respetando el filtro explícito si se pide.
- `documentosObra`: sin cambios, ya estaba correcto desde DEPT-01.
- No se tocó `alejandra-agente/worker.js`: esta lógica vive en los endpoints REST de `worker.js` que usa el panel/app web, no en las herramientas del agente (`consultar_bd`/`escribir_bd`), así que no aplica la regla "UNA Alejandra, DOS cerebros" a este cambio concreto.

**Verificación:**
- `node --check worker.js` OK; los 3 `<script>` de panel.html compilan sin error (`new Function()`).
- Preview con sesión simulada (`localStorage.panel_session` + reload): un encargado de departamento `electrico` tiene exactamente los 27 `data-page` de `ITEMS_OCULTOS_OFICIO` marcados `data-hidden-by-perms="true"` -ni uno más ni uno menos-, y ninguno de los ítems que debían quedarse (incidencias, tareas, planosObra, calendario...) aparece oculto. Un `jefe_de_obra` del mismo departamento NO tiene ninguno de esos 27 ítems oculto -confirma que mantiene el set completo-. `fotosObra` aparece una única vez en el DOM, dentro de "Obra".
- `git diff` sin corrupción de encoding nueva.
- Deploy de `worker.js`: `npx wrangler deploy` OK, Version ID `65e8e7cc-2c86-4d1c-a26b-43809445b9bf`, bindings DB/FILES presentes, `/health` responde `{"ok":true}` tras el deploy.
...
**Implementación (panel.html):**
- `AYUDA_SECCIONES`: diccionario único `data-page → descripción corta en español`, con una entrada para cada uno de los ~113 `data-page` del sidebar (verificado sin huecos comparando contra el DOM real vía `preview_eval`). Es la fuente única de texto reutilizada en 3 de las 4 vías, para no mantener descripciones duplicadas.
- **Vía 1 (tooltip):** `inicializarAyudaSidebar()` — nueva función que pone `.title` en cada botón `.nav-item[data-page]` a partir de `AYUDA_SECCIONES`. Se llama una vez desde `iniciarApp()`.
- **Vía 2 (estado vacío explicativo → adaptada a banner contextual):** se detectaron ~90 puntos de código distintos donde cada página renderiza su "sin datos" (placeholders de Tabulator, `<tr>` ad-hoc, divs `.empty-state`) — tocar cada uno individualmente era demasiado riesgo/esfuerzo para una sola sesión sin poder probar cada página a fondo. Decisión propia (bajo autorización de modo auto): un único banner contextual dismisseable (`#ayudaBanner`/`#ayudaBannerTexto`) encima de `#content`, que se actualiza en cada `navTo()` vía `_actualizarAyudaBanner(page)` con el texto de `AYUDA_SECCIONES[page]`, y que el usuario puede cerrar con "✕" (`ayudaBannerCerrar()`) — el cierre se recuerda en `localStorage` por sección (`ayuda_vista_<page>`), así no vuelve a molestar una vez leído. Cumple el mismo propósito (explicar cada sección) con un solo punto de código en vez de ~90.
- **Vía 3 (Alejandra lo explica por chat):** se descubrió que `alejandra-agente/worker.js` YA tenía un mecanismo `pantalla`/`canal` en `/api/chat` (usado hasta ahora solo por la app móvil, `index.html`) que mete `[Sesión: ... pantalla="P"]` en el contexto de sistema de Alejandra (`construirMessages()`). El panel nunca lo usaba. Cambio mínimo: `alejandrEnviar()` (el FAB de chat) ahora manda también `canal: 'panel'` y `pantalla: <título+descripción de AYUDA_SECCIONES de la página actual>` en el POST — reutiliza infraestructura ya desplegada, **no se tocó `alejandra-agente/worker.js`**.
- **Vía 4 (tour de onboarding):** `AYUDA_GRUPOS` (data-sid → descripción de grupo, 13 entradas) + funciones `iniciarTourSidebar()`/`_tourMostrarPaso()`/`_tourQuitarOverlay()`/`_tourSiguiente()`/`_tourFinalizar()`. Recorre solo las secciones actualmente visibles en el sidebar (respeta el filtrado por rol/departamento ya existente), mostrando un overlay con la descripción de cada grupo. Se lanza automáticamente una vez por usuario (`localStorage.tour_sidebar_visto`) vía `setTimeout(..., 700)` en `iniciarApp()`, y se puede volver a lanzar manualmente con un nuevo botón "❓" en la topbar (`iniciarTourSidebar(true)`).

**Verificación (preview, sesión simulada vía `localStorage.panel_session`+reload, rol superadmin):** cobertura de tooltips 100% sobre los `data-page` reales del DOM; banner cambia de texto al navegar entre páginas y se queda oculto tras cerrarlo, persistiendo por sección; botón "❓" relanza el tour bajo demanda incluso con el flag "visto" ya puesto; tour respeta qué secciones ve cada rol (no muestra pasos de secciones ocultas). Los 3 bloques `<script>` compilan sin error (`node -e "new Function(...)"`). Sin corrupción de encoding nueva (`grep "Ã|Â|â€|ï»¿"` sin coincidencias en líneas añadidas). `preview_screenshot` dio timeout repetido tras el reload simulado — diagnosticado como quirk del tooling de preview (los logs de consola no mostraban error real y `preview_eval`/`preview_snapshot` sí devolvían el DOM correcto), no un bug funcional; verificación completada con `preview_eval`/`preview_snapshot`/`preview_click` en su lugar.

**Alcance y despliegue:** solo `panel.html` (frontend estático de GitHub Pages) — no requiere `wrangler deploy`, solo push. Sin cambio de versión de app (panel.html no tiene `APP_VERSION` propio, mismo criterio que Part 25/23). No se tocó `worker.js` ni `alejandra-agente/worker.js` de forma funcional (solo se lee/reutiliza un parámetro que `alejandra-agente/worker.js` ya soportaba) — no aplica la regla "UNA Alejandra, DOS cerebros" porque no es un cambio de seguridad/permisos, solo el cliente panel empezando a mandar un dato que el otro lado ya sabía procesar.

**Pendiente:** ninguno explícito — las 4 vías aprobadas en DEPT-01 quedan implementadas en todo el sidebar. Si en el futuro se quiere abordar el estado vacío explicativo por-página (en vez del banner genérico), haría falta revisar los ~90 puntos de renderizado "sin datos" uno a uno.

### Part 25: SIDEBAR-01 — Reorganización del sidebar de panel.html (22/07/2026)
**Contexto:** Adrian: *"que mas podemos hacer... podemos mirar el lio con todos los paneles en Alejandra Office. podemos organizarlos de alguna manera. tambien teniamos pendiente el meter algo de ayuda para saber que es cada seccion y como funciona, recuerdas?"* — pedido doble: (1) ordenar el sidebar desordenado, (2) retomar la "descubribilidad de módulos" que quedó pendiente al cierre de DEPT-01 (Part 22).

**Diagnóstico:** la sección `📓 Seguimiento` del sidebar tenía 40 botones `nav-item` en fila, mezclando temas completamente distintos: finanzas (flujoCaja, facturasProveedor...), RRHH (ausencias, horariosObra, formacionObra), seguridad (ats, accidentes), maquinaria (partesMaquinaria, flotaVehiculos, alquileres, residuos), documentación de campo (partesPanel, diarioObra, fotosObra...), proveedores/subcontratas y calidad/cierre (licenciasObra, garantias, ensayos...). Al plantear agrupar esos 40 items en subsecciones visuales, Adrian preguntó *"ademas la seccion de Seguimiento porque?"* — se leyó el código real de `sidebarToggle()` y se confirmó que panel.html NO soporta anidamiento de 2 niveles: cada `.sidebar-section[data-sid]` es un desplegable de nivel único que mueve sus `nav-item` hermanos hasta la siguiente `.sidebar-section`. No tenía sentido mantener un wrapper "Seguimiento" con sub-grupos falsos.

**Decisiones de Adrian (recogidas por rondas de AskUserQuestion):**
1. Aprobó agrupar en 5 subsecciones temáticas, pero pidió ajustar nombres/grupos.
2. *"'personal y Seguridad' esos fuera, independientes"* — los items de RRHH y de seguridad NO deben formar parte de los grupos nuevos.
3. Confirmó mover esos items a las secciones "Personal" y "Seguridad" YA EXISTENTES en el sidebar (en vez de crear "Personal de Obra"/"Seguridad de Obra" nuevas).
4. *"Sí, quitar 'Seguimiento' y dejar 5 secciones nuevas independientes"* — eliminar el wrapper del todo, 5 secciones top-level en su lugar.
5. Sobre el texto de ayuda/descubribilidad: dijo dos veces que prefería explicarlo él mismo, pero no llegó a concretar el formato en esta sesión — queda completamente pendiente y sin definir.

**Implementación (panel.html):**
- Eliminado el bloque `<div class="sidebar-section" data-sid="seguimiento">` (40 botones) y sustituido por 5 secciones nuevas top-level, mismos botones/badges/`onclick` preservados verbatim, solo reordenados:
  - `📓 Diario y Campo` (`data-sid="diarioCampo"`, 8 items): partesPanel, diarioObra, fieldReports, fotosObra, correspondencia, actasReplanteo, registroHormigonado, cubicacionesObra.
  - `💰 Finanzas de Obra` (`data-sid="finanzas"`, 9 items, con clase `nav-finance` preservada): flujoCaja, facturasProveedor, cobrosCliente, catalogoPrecios, panelVencimientos, licitaciones, comparativosOferta, gastosDietas, changeOrders.
  - `🏭 Proveedores y Subcontratas` (`data-sid="proveedores"`, 7 items): proveedoresGestion, evaluacionesProv, caeDocumentacion, libroSubcontratacion, solicitudesMat, consumosMaterial, ordenesTrabajo.
  - `🚜 Maquinaria y Equipos` (`data-sid="maquinaria"`, 4 items): partesMaquinaria, flotaVehiculos, alquileres, residuos.
  - `🛡️ Calidad y Cierre` (`data-sid="calidadCierre"`, 7 items): licenciasObra, segurosObra, garantias, ensayos, rendimientos, lecciones, entregables.
- Sección `Personal` (existente): añadidos 3 botones tras "Turnos" — ausencias, horariosObra, formacionObra (sin clase especial, visibles para cualquiera que vea Personal).
- Sección `Seguridad` (existente): añadidos 2 botones tras "Permisos Trabajo" — ats, accidentes, CON la clase `nav-seguridad-section` + `style="display:none"` que ya usan el resto de items de esa sección (se revelan solo si `SESSION.departamento === 'seguridad'`, vía el bloque existente que hace `querySelectorAll('.nav-seguridad-section').forEach(el => el.style.display = '')`).
- `iniciarApp()` y `ocultarSidebarPorPermisos()` (los 2 sitios que gatean secciones para no-admin): `seccionesPermitidas`/`seccionesOcultar` cambian `'seguimiento'` por los 5 `data-sid` nuevos (`diarioCampo`, `finanzas`, `proveedores`, `maquinaria`, `calidadCierre`) — mismo comportamiento de permisos que antes (oculto para no-admin), solo cambian los nombres/agrupación.
- Comentario de código actualizado con la etiqueta `SIDEBAR-01` (se usó provisionalmente `ROLES-02` durante la implementación, corregido antes de cerrar la sesión).

**Efecto secundario real, comunicado a Adrian:** antes, `ats`/`accidentes` eran visibles para CUALQUIER admin dentro de "Seguimiento", sin importar el departamento de su sesión. Ahora, al vivir dentro de la sección "Seguridad" (gateada por departamento), solo se ven si `departamento === 'seguridad'` — mismo criterio que el resto de esa sección. Cambio de comportamiento aceptado como consecuencia de la decisión de mover esos items a la sección existente.

**Verificación:** recuento de los 40 `data-page` originales confirmado sin pérdida ni duplicado tras el reparto en los nuevos grupos; los 3 bloques `<script>` de panel.html compilan sin error (`node -e "new Function(...)"`); `git diff` sin corrupción de encoding nueva (grep `Ã|Â|â€|ï»¿` sin coincidencias en líneas añadidas). Probado en el preview local: un primer intento inyectando `window.SESSION = {...}` directamente vía `preview_eval` dio un falso positivo (las 5 secciones nuevas aparecían ocultas incluso para un superadmin simulado) — diagnosticado que `SESSION` es una variable `let` de módulo en panel.html (`let SESSION = JSON.parse(localStorage.getItem('panel_session') || 'null')`), no una propiedad de `window`, así que la inyección no llegaba a la función real `hasSessionRole()`. Repetido correctamente escribiendo en `localStorage.panel_session` + `location.reload()`: superadmin ve las 5 secciones nuevas + Personal (y Seguridad oculta por no tener `departamento:'seguridad'`, esperado); un `encargado` con `departamento:'seguridad'` ve solo la sección Seguridad (con ats/accidentes visibles dentro) y NO ve las 5 secciones nuevas (correcto, son admin-only). Sesión de prueba limpiada de localStorage al terminar.

**Alcance y despliegue:** solo `panel.html` (frontend estático de GitHub Pages) — no requiere `wrangler deploy`, solo push. Sin cambio de versión de app: panel.html no tiene `APP_VERSION` propio (mismo criterio que BUG-INVISIBLE, Part 23). No aplica a `worker.js` ni `alejandra-agente/worker.js` — es puramente reorganización de navegación, sin lógica de negocio ni permisos de backend.

**Pendiente, sin resolver:** el texto/ayuda de descubribilidad de módulos que motivó parte del pedido original de Adrian sigue sin definir — preguntó dos veces "prefiero explicarlo yo mismo" sin llegar a concretar formato (tooltip / estado vacío explicativo / Alejandra lo explica por chat / tour de onboarding) ni alcance (qué secciones). Retomar en una sesión futura preguntándole directamente, sin asumir ninguna de las 4 vías por defecto.
...
listados: `0 7`, `0 18` (deploy manual Version ID d9bcf385; el push posterior disparó el CI
...
### Part 14: Fixes menores de calidad en tools no-destructivas (SEC-06) (20/07/2026)
**Contexto:** Adrian: "hazlos" → aplicar los 3 fixes menores que quedaron diferidos en Part 13 (fuera del alcance "solo la red").

**Cambios en worker.js:**
1. `web_search` — valida que `query` no esté vacío y que exista `TAVILY_API_KEY`; devuelve error claro en vez de mandar `undefined` al API de Tavily.
2. `memory_save` — valida que `tipo`/`titulo`/`contenido` estén presentes; evita insertar NULLs en `alejandra_memoria`.
