...
**Sesion:** LIBRE
**Fecha:** 30/07/2026 -- Adrián reportó que Alejandra en la PWA (app_android) le devolvía el error crudo `Error: Anthropic 500: Internal server error` en vez de responder, justo al mandarle fotos de PEMPs de Liftisa para inventariar (2 de 3 intentos fallaron así esa madrugada, verificado leyendo `alejandra_historial` directo en D1). Causa: `fetchAnthropicConReintentos` (alejandra-agente/worker.js) ya reintentaba 429/500/502/503/529, pero el fallback a GPT-4o tras agotar reintentos solo cubría 429/529/503 -- un 500 persistente de Anthropic se propagaba tal cual como si fuera la respuesta del asistente. Fix: se añade 500 a la condición de fallback en `llamarAnthropic` y `llamarAnthropicStream` (mismo patrón, dos sitios). Commit `d926ca2`. npm test 85/85 verde, `wrangler deploy` verificado (bindings DB/FILES/RATE_LIMIT_KV OK) antes de push.
