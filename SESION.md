...
**PARA RETOMAR EN OTRO CHAT:** sesión cerrada limpia, sin trabajo a medias. Todo commiteado y pusheado a `main` (último commit `ffb7475`), worker desplegado y verificado. **Pendiente de que Adrián borre 2 empresas de prueba** que quedaron en D1 (ver SEC-AUDIT-03 abajo — datos ficticios, sin riesgo, pero sin borrar todavía; ahora con algún dato extra de fuzzing con el mismo prefijo `PENTEST-`, incluida una PEMP con matrícula en blanco anterior al fix, id 46 en empresa_id=6 — sin riesgo, mismo criterio). **La clase de bug "confusión de tipos" (`campo?.trim()` sin comprobar tipo) queda cerrada del todo** en las 210 apariciones del archivo, a petición explícita de Adrián. Sigue en curso "hasta que rompa" si Adrián quiere continuar: pendiente probar unicode/null-bytes y el mismo patrón de payload gigante contra otros endpoints con campos de texto libre.
