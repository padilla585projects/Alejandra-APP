...
**PARA RETOMAR EN OTRO CHAT:** sesión cerrada limpia, sin trabajo a medias. Todo commiteado y pusheado a `main` (último commit `48ad54c`), workers desplegados y verificados. **Pendiente de que Adrián borre 2 empresas de prueba** que quedaron en D1 (ver SEC-AUDIT-03 abajo — datos ficticios, sin riesgo, pero sin borrar todavía). **Sigue en curso "hasta que rompa"** (fuzzing/estrés a petición explícita de Adrián): límite de body (SEC-AUDIT-07) y JSON anidado/confusión de tipos ya probados y corregidos (4 sitios confirmados en vivo, ~25 más con el mismo patrón de bajo riesgo sin confirmar aún). Pendiente: unicode/null-bytes, y una pasada más amplia sobre los ~25 sitios restantes con `campo?.trim()` si Adrián quiere cerrarlo del todo.
...
**Seguimiento del mismo round — confusión de tipos:** siguiendo con fuzzing (JSON anidado 50.000 niveles, arrays/objetos donde se esperaba texto, `null`, números), el anidamiento profundo no rompió nada (V8 lo maneja bien, la validación normal de "faltan campos" lo atrapa), pero `nombre`/`codigo`/`titulo` como número disparaban un `TypeError` sin manejar (`campo?.trim is not a function`, 500 genérico) en `crearUsuario` y, confirmado en vivo también, en `crearObra`/`crearIncidencia`/`crearBobina` — mismo patrón (`campo?.trim()` asumiendo que el body trae un string) repetido en ~25 sitios más del archivo, no confirmados en vivo, de bajo riesgo (sin fuga de datos, solo un 500 en vez de una validación limpia). Arreglados los 4 confirmados exigiendo `typeof === 'string'` antes de validar. Verificado en vivo (5 repeticiones cada uno tras esperar la propagación del deploy): los 4 casos ya dan 400 con mensaje normal; peticiones legítimas siguen devolviendo 201/201/201/201 sin cambios. Commits `f272aac`, `48ad54c`.
