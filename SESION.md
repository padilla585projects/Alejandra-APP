...
**Sesion:** LIBRE
**Fecha:** 26/07/2026 -- Continuación de Part 58/59: tras el refactor "verComoAdmin" (v8.55), Adrián pidió reorganizar el sidebar de "Todos los departamentos" en un bloque colapsable por departamento en vez del listado plano de siempre (v8.56), con Albañilería/Pintura/Carpintería dentro del bloque de Obra Civil, no como bloques propios (v8.57). Aparecieron dos huecos reales al probarlo (stock de Seguridad huérfano bajo Almacén + las secciones no se contraían pese a v8.48) y se cerraron los dos junto con la causa raíz de que "no se contraían" desde v8.48 (v8.58). A petición de Adrián, Almacén pasó a ser "solo material" (todo el material de todos los departamentos + modal de filtro por departamento, Pedidos en su pestaña propia, sin Hoy en obra/Personal/Documentación, v8.59), extendiendo después el filtro del modal a Herramientas/Repostajes/Mantenimientos/Alertas Stock, que no tenían filtro de departamento propio (v8.60). Se arregló también la sección "Desarrollador" del sidebar, que era la única sin la estructura para plegarse con la flecha (v8.61). Un aviso de Adrián de que Ingeniería "no tenía planificación" resultó ser un parpadeo real (orden de ejecución: se ocultaba "Planificación" antes de mover Fases/Hitos/Planos/Cronograma a su bloque propio) — corregido invirtiendo el orden (v8.62). Revisando las secciones de Ingeniería a petición de Adrián, salieron 2 fugas más del mismo patrón "página nativa sin clase protectora, ningún departamento la reclama pero se queda visible": la sección "Obra" (Documentación y control genérica) sobraba para todo departamento con `controlObra:false` (v8.63), y "Seguridad de Obra" sobraba dentro del bloque Construcción para Ingeniería/Obra Civil (v8.64). Por último, Adrián detectó que colapsar "Desarrollador" (última sección del sidebar) se llevaba por delante el pie (versión + Cerrar sesión) al no haber ninguna sección después que parara el barrido, y que "Chat DevTools IA" reaparecía solo cada 3s por un reintento que no miraba el estado de plegado — ambos corregidos (v8.65).
...
**PARA RETOMAR EN OTRO CHAT:** sesión cerrada limpia, sin trabajo a medias. Todo commiteado y pusheado a `main` (último commit `44c5045`), worker desplegado y verificado. **Pendiente de que Adrián borre 2 empresas de prueba** que quedaron en D1 (ver SEC-AUDIT-03 abajo — datos ficticios, sin riesgo, pero sin borrar todavía; ahora con bastantes registros de prueba adicionales de las verificaciones de SEC-AUDIT-08, todos con datos de prueba obvios como "Test final"/"Conc-..."). **La clase de bug "confusión de tipos" queda cerrada del todo** (210 apariciones) y **la condición de carrera de numeración también queda cerrada del todo** (39 endpoints en total entre SEC-AUDIT-05 y SEC-AUDIT-08), ambas a petición explícita de Adrián de "arreglar todo". De paso se encontró y corrigió un bug preexistente sin relación (json con status como objeto) que rompía 5 módulos completos. Sigue en curso "hasta que rompa" si Adrián quiere continuar: pendiente probar unicode/null-bytes de forma más agresiva (ya probado un caso básico, sin problemas) y seguir buscando otras clases de bug con el mismo criterio sistemático.
