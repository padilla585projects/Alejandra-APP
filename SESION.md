...
**Sesion:** EN CURSO
**Fecha:** 21/07/2026 -- Auditoría de modelos IA y costes: fix pricing Opus 4.6 (COST-01), evaluadas y descartadas migraciones buscar_web/MODEL_EXPERTO (COST-02/03), ahora diseñando Batch API para ejecutarReflexion (COST-04)
**Versión actual:** v7.98 (sin cambio de versión de app -- solo backend)
**Resumen:** Adrian pidió revisar modelos OpenAI/Anthropic en uso, nuevos modelos disponibles, deprecaciones, y cómo ahorrar coste (Anthropic "muy caro"). Auditoría con fuentes oficiales (platform.claude.com, developers.openai.com) tras detectar que WebSearch traía resultados poco fiables de agregadores SEO. Hallazgo real corregido: AI_PRICES['claude-opus-4-6'] en worker.js tenía el precio de Opus 4.1 (deprecado) en vez del de Opus 4.6 real ($5/$25, no $15/$75) -- fix desplegado. Las otras 2 migraciones propuestas (buscar_web a gpt-5.4-nano, MODEL_EXPERTO a claude-sonnet-5) se investigaron a fondo y se DESCARTARON: ninguna da ahorro real hoy (ver COST-02/COST-03 en IDEAS_PENDIENTES.txt para el detalle). Queda en curso el diseño de Batch API para el cron ejecutarReflexion (COST-04), único ahorro real identificado y significativo (~95% combinado con el prompt caching ya existente).
