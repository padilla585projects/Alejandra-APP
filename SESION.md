...
**Sesion:** LIBRE
**Fecha:** 27/07/2026 -- Sesión larga de seguridad y estrés (Parts 60+, ver SEC-AUDIT-01 a 08 más abajo): auditoría completa a petición de Adrián ("la seguridad en la app... alguien con malos pensamientos"), con 3 críticos reales (escalada empresa_admin→superadmin confirmada en vivo, IDOR cross-empresa, envenenamiento de memoria de la IA sin auth) y una campaña larga de "poner al límite" (pentest de estrés, fuzzing, hasta 1000 peticiones concurrentes reales, simulación de jornada con 8 usuarios a la vez) que destapó y cerró: condición de carrera de numeración en 39 endpoints, condición de carrera en devoluciones de bobinas/PEMP/carretillas, dos clases enteras de crash por confusión de tipos (210+ sitios, cerradas con un patrón global reutilizable en vez de campo a campo), un DoS de bajo coste por payload gigante, y varios bugs preexistentes sin relación (json con status como objeto, horarios_obra borrando campos en updates parciales). Informe final consolidado publicado como artefacto (no commiteado al repo, a petición de Adrián — un informe con detalle de vulnerabilidades no debe quedar en el propio repo).
