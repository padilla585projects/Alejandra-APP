...
**No tocado, riesgo bajo o decisión de diseño ya documentada en el código:** `/api/esquemas/view/` sirve sin auth por diseño explícito (enlaces de diagramas compartibles); 7 módulos (`licencias-obra`, `catálogo-precios`, `seguros-obra`...) llaman a funciones no definidas (`getEmpresaFromRequest`/`jsonOk`/`jsonError`) y dan 500 siempre — no explotable ahora, pero si se reactivan hay que definirlas con el patrón `getAuth`/`json`/`err` ya existente, no una función nueva sin auditar.

**Deploy:** `npx wrangler deploy` en ambos workers, `/health` verificado 200 en los dos tras el deploy. Commit `956086e`.
