...
**Sesion:** LIBRE
**Fecha:** 25/07/2026 -- Continuación en caliente de la sesión de pruebas con Alberto (Bobinas + chat interno), Parts 57→58, seguida de la checklist completa de pendientes (v8.28→v8.40), y luego una ronda de pruebas en vivo en un HTC real por ADB (con permiso de Adrián — él escribía sus propias credenciales, Claude solo navegaba/miraba capturas): fix de galería de fotos en SEG-02 (v8.41), barra superior demasiado apretada + franja naranja de status bar + nombre de obra truncado + chip "SA" quitado (v8.42→v8.45), y un bug real de solapamiento visual encontrado y arreglado en el selector de departamento (v8.46). v8.46.
**Versión actual:** v8.46 (version.json/sw.js/index.html sincronizados)
**Resumen:** Parts 50-56 = roles/departamentos (hasta v8.23). Part 57 = primera tanda de bugs en vivo con Alberto (hasta v8.27). Part 58 (esta sesión) = segunda tanda + checklist de pendientes + pruebas en HTC real, v8.28→v8.46. Ver Parts 50-58.

**PENDIENTE (checklist para retomar):**
- **Departamentos sin menú propio en Office:** Obra Civil, Albañilería, Pintura, Carpintería, Telecom, Almacén — Adrián confirmó que ninguno tiene cuentas reales todavía ("pero tendremos que estar preparados para cuando pase"). No hay nada roto: sin config en `_MENU_ROL_DEPT_CONFIG`, el sidebar se deja completo/sin curar (fallback seguro). Gracias al refactor de INGENIERIA-02 (Part 58), añadir el menú de cualquiera de estos departamentos cuando haga falta es solo una entrada nueva en el config, sin tocar `_organizarSidebarRolDept()`.
