...
**Sesion:** LIBRE
**Fecha:** 25/07/2026 -- Continuación en caliente de la sesión de pruebas con Alberto (Bobinas + chat interno), Parts 57→58. Adrián y Alberto siguieron probando bobinas/chat en producción en tiempo real; salieron y se arreglaron sobre la marcha: fila "Registrado por" rompiendo el layout de la tabla, FAB de Alejandra/chat interno que no aparecía tras recargar (2 intentos hasta dar con la causa real), filtros de Estado/Nº albarán + impresión completa en Bobinas, historial de bobinas visible en Office (ya existía en la app móvil), archivado automático de devueltas a los 7 días, filtro de obra inútil oculto para roles sin permiso, aviso (badge+parpadeo+pitido) de mensajes de chat privados sin leer, y el chat DevTools IA movido del botón flotante al sidebar. v8.36.
**Versión actual:** v8.36 (version.json/sw.js/index.html sincronizados)
**Resumen:** Parts 50-56 = roles/departamentos (hasta v8.23). Part 57 = primera tanda de bugs en vivo con Alberto (hasta v8.27). Part 58 (esta sesión) = segunda tanda, Bobinas + chat interno + notificaciones, v8.28→v8.36. Ver Parts 50-58.

**PENDIENTE (checklist para retomar):**
- **Chat interno en la app móvil**: sigue sin selector de contacto para privado 1 a 1 (solo Office lo tiene) — la app cae bien al canal "Todos", pero no hay paridad completa. El aviso de no-leídos (badge/pitido, Part 58) tampoco se construyó en la app, solo en Office.
...
- **NUEVO (Part 58) — "revisa que todos los cambios de inventarios se hagan en tiempo real entre app y office"**: Adrián lo pidió explícitamente durante la sesión y quedó sin investigar (se priorizaron los bugs que iban saliendo en directo). Sigue pendiente.
- **NUEVO (Part 58) — "quiero quitar columnas del inventario que no valen y añadir otras"**: pedido por Adrián, nunca llegó a especificar cuáles. Preguntar antes de tocar la tabla de Bobinas otra vez.
- **NUEVO (Part 58) — cuenta duplicada de Alberto**: existen 4 usuarios "Alberto" en D1 (ids 1, 12, 46, 50, 51) — solo el 46 (`Alberto Martínez`, amartinezc@levitec.es) está activo y en uso real; el resto están `activo=0`. No se ha hecho limpieza, solo se identificó al depurar por qué un mensaje de chat "no llegaba" (sí llegaba, al id correcto). Revisar si conviene borrar/desactivar formalmente los duplicados en una sesión de datos.
- **NUEVO (Part 58) — el archivado automático de bobinas devueltas (7 días) es solo un filtro de vista** (`_bobinaEstaArchivada()` en cargarBobinas()), no borra ni mueve filas de la tabla `bobinas`. El histórico real vive en la tabla `historial` (endpoint `/historial`, ya existía). Si en el futuro se quiere cambiar el nº de días, está en la constante `BOBINAS_DIAS_ANTES_DE_ARCHIVAR` en panel.html.
