...
**Sesion:** LIBRE
**Fecha:** 25/07/2026 -- Continuación en caliente de la sesión de pruebas con Alberto (Bobinas + chat interno), Parts 57→58. Adrián y Alberto siguieron probando bobinas/chat en producción en tiempo real; salieron y se arreglaron sobre la marcha: fila "Registrado por" rompiendo el layout de la tabla, FAB de Alejandra/chat interno que no aparecía tras recargar (2 intentos hasta dar con la causa real), filtros de Estado/Nº albarán + impresión completa en Bobinas, historial de bobinas visible en Office (ya existía en la app móvil), archivado automático de devueltas a los 7 días, filtro de obra inútil oculto para roles sin permiso, aviso (badge+parpadeo+pitido) de mensajes de chat privados sin leer, y el chat DevTools IA movido del botón flotante al sidebar. Tras cerrar esa tanda, Adrián pidió seguir con el resto de pendientes: sync en tiempo real de inventarios app↔Office (v8.37) y chat privado 1 a 1 + aviso de no-leídos en la app móvil, dando paridad completa con Office (v8.38). v8.38.
**Versión actual:** v8.38 (version.json/sw.js/index.html sincronizados)
**Resumen:** Parts 50-56 = roles/departamentos (hasta v8.23). Part 57 = primera tanda de bugs en vivo con Alberto (hasta v8.27). Part 58 (esta sesión) = segunda tanda, Bobinas + chat interno + notificaciones + pendientes, v8.28→v8.38. Ver Parts 50-58.

**PENDIENTE (checklist para retomar):**
- **`#remoteScanFab` no es arrastrable** (a propósito, contenedor con sub-botones — ver Part 57).
- **`_parseFechaBackend()` (SCAN-03) sigue siendo un parche en el formateador de Office**, no en el origen (`fechaEspana()` en worker.js sigue guardando fechas en formato español no-ISO).
- **Departamentos sin menú propio en Office todavía:** Obra Civil, Albañilería, Pintura, Carpintería, Telecom, Almacén — sin confirmar que tengan cuentas reales.
- **Ingeniería** sigue con menú por departamento, no por el rol "Ingeniero" nuevo.
- **SEG-02 (Registro de Seguridad) sin probar con login real todavía.**
- **Asunciones de privacidad sin confirmar campo a campo** (Recursos Preventivos/Operario, Parts 51/56).
- **Resend sigue en dominio sandbox** (`noreply@resend.dev`).
- **Sugerencias de la app**: sin revisar desde la #208.
